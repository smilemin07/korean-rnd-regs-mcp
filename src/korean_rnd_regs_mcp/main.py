...
_SERVER_INSTRUCTIONS = (
    ...
    "도구 호출 결과 질문 대상이 지원 60개 규정 밖이면 본 서버 근거로 확인되지 않았음을 밝히고, "
    ...
    "별표 내용을 표·목록 등으로 정리해 표시한 답변은 답변 전에 인용·재구성·요약 중 "
...
async def get_provision_detail(provision_id: str, include_old_and_new: bool = False, annex_chunk: int | None = None, annex_locate: str | None = None) -> dict:
    """사용 시점: search_provision 또는 suggest_review_sources가 반환한 provision_id의 원문·삭제 여부·현행 내용을 확인할 때 호출하십시오. provision_id 없이 조문 내용을 추측하지 마십시오. 이 도구의 content가 규정 조문·별표 본문의 권위 출처이므로, 본문은 외부 웹(law.go.kr 직접 열람·웹검색 결과)에서 가져오지 말고 이 도구로 확인하십시오. content_format이 plain_text_verbatim이 아니면 응답이 제공한 attached_file_url·document_source_url의 공식 원문을 확인하십시오. 행정규칙(admrul) 응답에는 발령번호·종류가 issuance_number·regulation_kind·version_label 필드로 포함되니 이를 사용하되, 이 값은 조회된 규정의 것이며 현행임을 보증하지 않으므로(검색 실패 시 등록 버전일 수 있음) 현행 여부 단정이 필요하면 1차 출처에서 확인하고, 응답에 없는 고시·예규 번호 등은 외부 값으로 단정하지 마십시오. 기한·금액·비율·수치 등 구체값도 마찬가지로, 응답 원문에 있는 값은 그대로 인용하되 원문에서 확인되지 않은 값은 임의 예시로라도 단정하지 말고 확인되지 않았음을 명시하십시오. 지원 범위 내 질문에 답하면서 지원 범위 밖 법령·행정규칙의 조문번호·요건·효과 등 구체 내용을 보조 맥락으로 덧붙일 때도 마찬가지로, 도구 응답 원문에서 확인되는 부분이 아니면 일반 학습지식에 따른 설명임을 명시하고 그 내용을 현행 사실로 단정하지 마십시오(보조 설명 자체는 허용 — 출처 구분 표시 요구).
    ...
    - unit_id가 JO… 면 조문 본문 + article_structure (v0.6.0 size-tiered — 대용량 조문은
      ...
      일부만 확인했는지 확인 범위를 답변에 명시할 것. 별표 내용을 표·목록 등으로 정리해 표시한 답변은
...
- Tier 2 (사업 운영규정·요령): 국토교통부소관 연구개발사업 운영규정, 산업기술혁신사업 공통 운영요령, 산업기술혁신사업 보안관리요령, 산업기술혁신사업 기술개발 평가관리지침, 중소기업기술개발 지원사업 운영요령, 기술료 징수 및 관리에 관한 통합요령(산업부), 중소기업기술개발 지원사업 기술료 관리규정(중기부), 보건의료기술 연구개발사업 운영·관리규정(보건복지부), 국방기술 연구개발 업무처리지침(방위사업청), 국방과학 기술료 산정ㆍ징수방법 및 징수절차 등에 관한 고시(방위사업청)
...
3. search_provision(query=...)으로 추가 검색 및 주제별 cross-check
   ...
   - 공통/사업 행정규칙 cross-check (해당 시):
     ...
     국방 R&D/국방연구개발/국방과학기술/국방기술/방위사업청/방사청/무기체계 연구개발 → defense_tech_act · defense_tech_decree · defense_tech_rule · defense_rnd_guideline · defense_tech_fee_notice
