...
def _build_annex_detail(provision_id: str, unit_id: str, rs, ann: dict, eff_date: str) -> dict:
    ...
    if len(content) <= 60 and "삭제" in content and "<" in content:
        base.update({
            "content": content,
...
async def get_provision_detail(provision_id: str) -> dict:
    ...
    - unit_id가 JO… 면 조문 본문 + article_structure, BP… 면 별표 본문 (행정규칙·법령 시행령 모두, v0.2; size-tiered)
    """
    try:
    ...
    if pid.unit_id is None:
        ...
        result["annexes_count"] = len(annexes)  # v0.2: 법령도 별표 보유 → BP 발견 경로
        if _revision:
