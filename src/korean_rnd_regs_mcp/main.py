...
_SERVER_INSTRUCTIONS = (
    ...
    "도구 호출 결과 질문 대상이 지원 46개 규정 밖이면 본 서버 근거로 확인되지 않았음을 밝히고, "
...
# 사이징 32: N=46 cold peak 동시성 ≈46 → 32는 일부 잠깐 큐잉(~5s대)·48은 law.go.kr 동시연결
...
# 크게 상회하는 방어적 cap. 현행 43규정 최대 117조문이라 평시 미도달. 출력 목록 크기를 600으로 cap해
...
async def get_provision_detail(provision_id: str) -> dict:
    ...
    if pid.unit_id is None:
        ...
        # separator·메타데이터 누적 오차에 안전). 현행 43규정 최악 117조문 ~12.9k자라 평시 미발동.
