...
_SERVER_INSTRUCTIONS = (
    ...
    "도구 호출 결과 질문 대상이 지원 66개 규정 밖이면 본 서버 근거로 확인되지 않았음을 밝히고, "
    ...
    "본 서버는 규정 원문 도구 외에 「국가연구개발혁신법 매뉴얼」(본권·별권 3 「국가연구개발사업 제재처분 가이드라인」·별권 2 「국가연구개발사업 기술료 제도 매뉴얼」·별권 1 「학생인건비통합관리 제도 매뉴얼」·별권 4 「연구시설･장비비 통합관리제 운영･관리 매뉴얼」)과 「국가연구개발 과제평가 표준지침」(과학기술정보통신부·2025.12)·「국가 R&D 연구비 부적정집행 사례집」(국토교통과학기술진흥원(KAIA)·2025.05) 해설·지침·사례 조회 도구 search_manual·get_manual_section을 제공합니다. "
    "혁신법령 관련 실무 해설·세부 절차·Q&A·사례 설명이나 과제평가(선정·단계·특별·최종) 기준·절차 안내, 연구비 부적정집행 사례(비목별 정산 불인정 사례·FAQ) 확인이 필요하면 search_manual로 절을 찾고 get_manual_section으로 본문을 확인하십시오"
    "(검색은 본권·별권 3·별권 2·별권 1·과제평가 표준지침·별권 4·부적정집행 사례집을 함께 훑으며 매치의 source·citation으로 어느 자료인지 구분됩니다. 제재처분 기간·비율 등 구체값은 시행령 별표 6·별표 7 원문을, 기술료율·납부 상한 등 구체값은 시행령 제38조~제41조 원문을, 학생인건비·연구시설장비비 계상기준·적립·사용 요건 등 구체값은 연구개발비 사용 기준 원문을 get_provision_detail로 교차 확인. 과제평가 표준지침은 각 부처 평가지침의 기준이므로 구체 과제에는 소관 부처·전문기관의 평가지침·공고가 함께 적용됨을 안내. 부적정집행 사례집은 KAIA 발간 교육·참고용 자료로 개별 사안에 대한 정산 판정·제재처분 결정이 아니고 사례 결론은 기관 유형·협약·증빙에 따라 달라질 수 있으며, Ⅳ장 점검·정산 절차는 KAIA 국토교통R&D 프로세스 기준이므로 타 부처 과제는 소관 전문기관 절차를 확인하도록 안내). "
    "국토교통부(국토부) 소관 R&D 과제의 연구비 집행·정산·부적정집행 관련 질의에는 "
    "「국가 R&D 연구비 부적정집행 사례집」을 우선 확인 대상에 포함하십시오. "
    "단 이때 search_manual의 query에 '국토교통부'·'국토부' 등 부처명을 넣지 마십시오 — "
    "사례집 본문에 부처명 표기가 없거나 드물어 부처명 토큰이 든 검색은 0건이 되기 쉽습니다. "
    "부처명 대신 '부적정집행'과 해당 비목명(학생인건비·연구수당 등)·'정산' 같은 주제어로 검색하십시오. "
    "사례집은 KAIA가 국토교통R&D 관점의 안내를 포함해 발간했으나 사례는 국가 R&D 전반에서 수집된 자료이므로, "
    "국토교통부 전용 자료로 한정하지 말고 타 부처 과제 질의에서도 참조할 수 있습니다. "
...
_MANUAL_SUPPLEMENTS: list[dict] = [
    ...
    {
        # v0.39.0 R5: 별권 4 — rank는 append-only(수록 순·재정렬 금지). 장 없는 평면 편제라
        # id는 단일 레벨(b4-0 개요·b4-1~b4-9 Ⅰ~Ⅸ·b4-ref-1 붙임 — 계획 /disc 3/3).
        "source_id": "b4",
        "source_rank": 5,
        "prefix": "b4-",
        "loader": load_manual_b4,
        "error_code": "manual_b4_unavailable",
        "format_note": MANUAL_FORMAT_NOTE_B4,
        "label": "별권 4(연구시설·장비비 통합관리제 운영·관리 매뉴얼)",
        "warn_label": "별권 4",
        "unavailable_guidance": (
            "연구시설·장비비 통합관리 근거가 필요하면 "
            "규정 트랙(get_provision_detail·국가연구개발사업 연구개발비 사용 기준 제100조~제111조)을 "
            "사용하고, 매뉴얼 원문은 KISTEP 홈페이지(www.kistep.re.kr)에서 확인하도록 안내하십시오."
        ),
        "valid_range": (
            "별권 4(연구시설·장비비 통합관리제 운영·관리 매뉴얼) 유효 범위: 개요 b4-0 · "
            "본문(Ⅰ~Ⅸ) b4-1~b4-9 · 붙임 b4-ref-1"
        ),
    },
    {
        # v0.43.0: 매뉴얼 트랙 최초의 기관(KAIA) 발간 사례집 소스 — rank는 append-only(수록 순·
        # 재정렬 금지). 원문이 로마숫자 장(Ⅰ.~Ⅳ.) 2레벨 편제라 id는 case-장-절.
        "source_id": "case",
        "source_rank": 6,
        "prefix": "case-",
        "loader": load_manual_case,
        "error_code": "manual_case_unavailable",
        "format_note": MANUAL_FORMAT_NOTE_CASE,
        "label": "「국가 R&D 연구비 부적정집행 사례집」(KAIA)",
        "warn_label": "부적정집행 사례집",
        "unavailable_guidance": (
            "연구개발비 집행 기준 근거가 필요하면 "
            "규정 트랙(get_provision_detail·국가연구개발사업 연구개발비 사용 기준)을 사용하고, "
            "사례집 원문은 KAIA 홈페이지(www.kaia.re.kr)에서 확인하도록 안내하십시오."
        ),
        "valid_range": (
            "부적정집행 사례집 유효 범위: Ⅰ장 case-1-1 · Ⅱ장(비목별 사례) case-2-1~case-2-10 · "
            "Ⅲ장(FAQ) case-3-1 · Ⅳ장(점검·정산 절차) case-4-1"
        ),
    },
]
...
def _supplement_unavailable_envelope(desc: dict, err: ManualLoadError, main_ok: bool | None = None) -> dict:
    ...
    main_ok=True일 때만 본권 확인 문구 포함. ★v0.39.0 문면 정비: 종전 "규정 도구는 정상입니다"는
    그 시점에 규정 도구를 검사하지 않은 미확인 단정이라(v0.38.0 적대검토 발굴 backlog), 검증된
    아키텍처 사실(격리 — 이 오류가 규정 도구 경로로 전파되지 않음)만 말하는 문면으로 교체.
    """
    isolation_clause = (
        "이 오류는 해당 매뉴얼 데이터 로드에 한정되며 규정 도구"
        "(search_provision·get_provision_detail 등) 경로에는 전파되지 않습니다. "
    )
    main_clause = (
        "본권 매뉴얼 데이터는 정상 로드되었습니다. " + isolation_clause if main_ok else isolation_clause
    )
    return {
        "errors": [{
...
async def search_manual(query: str) -> dict:
    """사용 시점: 「국가연구개발혁신법 매뉴얼」 본권(범부처 공통 해설서)·별권 3 「국가연구개발사업 제재처분 가이드라인」·별권 2 「국가연구개발사업 기술료 제도 매뉴얼」·별권 1 「학생인건비통합관리 제도 매뉴얼」·별권 4 「연구시설･장비비 통합관리제 운영･관리 매뉴얼」·「국가연구개발 과제평가 표준지침」(과학기술정보통신부·2025.12)·「국가 R&D 연구비 부적정집행 사례집」(국토교통과학기술진흥원(KAIA)·2025.05)에서 혁신법령의 실무 해설·세부 절차·Q&A·사례·제재처분 절차와 기준 해설·기술료 제도 해설·학생인건비통합관리 해설·연구시설·장비비 통합관리 해설·과제평가(선정·단계·특별·최종) 기준과 절차·연구비 부적정집행 사례(비목별 정산 불인정 사례·FAQ)를 찾을 때 호출하십시오. 이 도구는 법령·행정규칙 조문 원문 검색이 아닙니다 — 조문 원문·현행 여부 확인은 search_provision·get_provision_detail을 사용하고, 매뉴얼·지침·사례집과 법령·행정규칙 내용이 다르면 법령·행정규칙 원문이 우선합니다. 국토교통부(국토부) 소관 과제의 연구비 집행·정산·부적정집행 질의에는 부적정집행 사례집을 우선 확인 대상에 포함하되, query에 '국토교통부'·'국토부' 등 부처명을 넣지 말고(사례집 본문에 부처명 표기가 없거나 드물어 0건이 되기 쉽습니다) '부적정집행'과 해당 비목명(학생인건비 등)·'정산' 같은 주제어로 검색하십시오. 사례는 국가 R&D 전반에서 수집된 자료이므로 국토교통부 전용으로 한정하지 마십시오(단 Ⅳ장 점검·정산 절차는 KAIA 국토교통R&D 프로세스 기준).

    본권(43개 절·인쇄 1~332쪽·부록 제외)·별권 3(23개 단위·인쇄 1~89쪽)·별권 2(15개 단위·
    인쇄 1~12쪽)·별권 1(26개 단위·인쇄 3~91쪽)·과제평가 표준지침(15개 단위·인쇄 1~60쪽)·
    별권 4(11개 단위·인쇄 3~50쪽)·부적정집행 사례집(13개 단위·인쇄 6~95쪽)을
    함께 절 단위로 검색합니다. 매칭은
    search_provision과 동일한 토큰 AND(공백 분해·2자 이상 토큰 2개 이상이면 모든 토큰 존재 시
    매칭, 그 외 리터럴)이며, 가운뎃점 표기차(ㆍ·･·)는 매칭에서 흡수합니다(발췌는 원문 그대로).
    정렬은 제목 매치 우선·같은 단계에서는 본권, 그다음 별권 3·별권 2·별권 1·과제평가 표준지침·별권 4·부적정집행 사례집 순입니다.

    응답: matches[](source["main"=본권|"b3"=별권 3|"b2"=별권 2|"b1"=별권 1|"eval"=과제평가 표준지침|"b4"=별권 4|"case"=부적정집행 사례집]·절 메타·인쇄쪽 범위·매치
        ...
        for i, (desc, d, _ok) in enumerate(supp_states):
            ...
            if i == len(supp_states) - 1:
                msg += (
                    ...
                    "로드 불가합니다. 이 오류는 매뉴얼 데이터 로드에 한정되며 규정 도구"
                    "(search_provision·get_provision_detail 등) 경로에는 전파되지 않습니다. "
                    "매뉴얼·지침 원문은 KISTEP 홈페이지(www.kistep.re.kr)에서, 부적정집행 사례집 "
                    "원문은 KAIA 홈페이지(www.kaia.re.kr)에서 확인하도록 안내하십시오."
                )
...
async def get_manual_section(section_id: str, chunk: int | None = None) -> dict:
    """사용 시점: search_manual로 찾은 「국가연구개발혁신법 매뉴얼」 본권·별권 3 「국가연구개발사업 제재처분 가이드라인」·별권 2 「국가연구개발사업 기술료 제도 매뉴얼」·별권 1 「학생인건비통합관리 제도 매뉴얼」·별권 4 「연구시설･장비비 통합관리제 운영･관리 매뉴얼」·「국가연구개발 과제평가 표준지침」·「국가 R&D 연구비 부적정집행 사례집」 절의 본문 전문이 필요할 때 호출하십시오. 이 도구는 해설·지침·사례 자료 조회이며 법령·행정규칙 원문 조회가 아닙니다 — 조문 원문은 get_provision_detail을 사용하고, 매뉴얼·지침·사례집과 법령·행정규칙 내용이 다르면 법령·행정규칙 원문이 우선합니다.
    ...
    별권 1 id "b1-장-절"(예: "b1-3-4"=제3장 4. 학생인건비 지급액 책정 및 지급·"b1-5-1"=제5장
    FAQ)·"b1-ref-1"~"b1-ref-3"(참고),
    별권 4 id(장 없는 평면 편제라 단일 레벨): "b4-0"(개요)·"b4-1"~"b4-9"(본문 Ⅰ~Ⅸ·예:
    "b4-5"=Ⅴ. 통합 연구시설･장비비의 계상･지급･적립)·"b4-ref-1"(붙임 관련 규정 발췌)
    또는 부적정집행 사례집 id "case-장-절": "case-1-1"(Ⅰ. 연구비 사용전 기억해야 하는 사항)·
    "case-2-1"~"case-2-10"(Ⅱ. 비목별 사례 — 예: "case-2-2"=학생인건비)·"case-3-1"(Ⅲ. FAQ)·
    "case-4-1"(Ⅳ. 상시·연차점검/정산 절차 — KAIA 프로세스 기준).
    ...
    별권 1·별권 4의 계상기준·지급액·적립·사용 요건 등 구체값 인용 시에는 연구개발비 사용 기준
    (rnd_funding_standard·연구시설·장비비 통합관리는 제100조~제111조) 원문을,
    부적정집행 사례집의 사용 기준·한도 등 구체값 인용 시에도 같은 연구개발비 사용 기준 현행
    원문을 교차 확인하십시오(사례집은 발간 시점 스냅샷·개별 사안 판정 아님).
        ...
        return {
            "errors": [{
                ...
                "message": _sanitize_error_message(
                    ...
                    "별권 1 'b1-장-절'(예: 'b1-3-4')·'b1-ref-N'(예: 'b1-ref-1'), "
                    "과제평가 표준지침 'eval-장-절'(예: 'eval-3-2')·'eval-ref-N'(예: 'eval-ref-6'), "
                    "별권 4 단일 레벨 'b4-N'(예: 'b4-0', 'b4-5')·'b4-ref-1'(장 없는 평면 편제라 "
                    "'b4-1-1'형은 유효하지 않음) 또는 "
                    "부적정집행 사례집 'case-장-절'(예: 'case-2-2', 'case-3-1'). "
...
@mcp.prompt(
    ...
    description=(
        ...
        "「국가연구개발혁신법 매뉴얼」(본권·별권 3 제재처분 가이드라인·별권 2 기술료 제도 매뉴얼·별권 1 학생인건비통합관리 제도 매뉴얼·별권 4 연구시설･장비비 통합관리제 운영･관리 매뉴얼)과 「국가연구개발 과제평가 표준지침」(25.12) 해설·지침은 매뉴얼 도구(search_manual·get_manual_section)로 조회 가능(해설·지침 자료 — 법령·행정규칙 아님·법령 우선). "
        "기관 내부 기준·본 manifest 미등록 전문기관 지침·기타 부처별 매뉴얼은 본 server 미커버 — 별도 자료 확인 필요."
    ),
)
