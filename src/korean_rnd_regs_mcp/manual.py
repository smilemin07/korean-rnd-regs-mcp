...
# v0.45.0: 문면 교체 — v0.44.0 eval에서 응답 구조 언급 차단은 6/6 달성했으나 발간처 오귀속은
# 잔존(High·Extra 4관측 재현: "원문 자체에 이렇게 명시되어 있습니다" 등). 구조 언급 금지(단순
# 생략 과제)와 달리 귀속은 호스트가 발화 주체를 새로 골라야 하는 과제라, 부정문만으로는 부족하고
# 긍정 대안(지정 귀속 문구)이 필요하다는 진단(계획 /disc 2/2). 지정 문구는 Andy 확정
# "korean-rnd-regs-mcp에서 제공하는 정보에 따르면," — 서버명 명명은 전달 메커니즘 지칭
# ('시스템 메타데이터' 등)과 달리 정보 작성 주체의 신원이라 자기지시성 노출이 아니다.
# law_priority_note 본문(v0.28.0 잠금)은 byte 불변 — 이 상수의 값만 바뀐다(직렬화 -3자·예산 무해).
