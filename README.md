...
| `search_manual`          | 「국가연구개발혁신법 매뉴얼」(본권) 해설 검색 — 절 단위 매치 + 인쇄쪽 앵커 발췌 (v0.27.0)                     |
| `get_manual_section`     | 매뉴얼 절 본문 조회 — 대형 절은 페이지 경계 분할 청크. 모든 응답에 규범성 메타(해설 자료·법령 우선) 동반 (v0.27.0) |
...
1. **claude.ai 웹 커넥터**: 설정에서 커넥터를 **비활성화 → 재활성화(재연결)** 한 뒤 **새 대화**에서 사용하세요. 진행 중이던 대화는 이전 도구 목록을 유지할 수 있습니다.
