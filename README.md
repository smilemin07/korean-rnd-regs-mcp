...
- **지원 범위 = 52개 규정**(혁신법 family·부처별 R&D·핵심 행정규칙). 이 범위 밖 규정에 대한 답변은 일반 지식 기반 안내이니, [국가법령정보센터](https://www.law.go.kr) 등 1차 출처를 함께 확인하세요.
...
| `list_rule_sets`         | 등록된 52개 규정 목록·hierarchy rank·문서 ID·소관부처 조회                                                   |
...
| Prompt                | 용도                                                                                                                                                                                          |
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `review_regulation` | **다층적 규정 검토** — 특정 사례에 대해 52개 규정을 위계 순서로 적용하여, 규정 검토 결과를 근거 조항 인용과 함께 답변. 'korean-rnd-regs-mcp', '규정 검토' keyword를 prompt에 포함 필요 |
...
- 검색 결과에서도 조문의 최근 개정 이력(공포일)이 보이도록 개선 — "최근 개정된 조문"을 검색만으로 파악하기 쉬워짐(지원 규정 수 52개 불변)
  - 검색(`search_provision`)·추천(`suggest_review_sources`) 결과의 법령 조문에 최신 이력 힌트 `latest_history`(예: "개정 2025.12.30(공포)")를 함께 표시. 종전에는 이 힌트가 문서레벨 조회에만 있어, 검색으로 먼저 다가가는 경우 개정 신호를 바로 보지 못했음
  - 검색 결과의 힌트는 키워드에 걸린 조문에 한정되므로, 특정 법령의 개정 조문 전수 확인은 문서레벨 조문 목록으로 안내
  - 날짜는 공포일(시행일이 아님)이며, 힌트가 없는 조문을 "개정되지 않았다"로 단정하지 않도록 안내 병기

### 2026. 7. 8. : v0.15.0

- 조문별 최근 개정 이력(공포일) 발견성 추가 — "최근 개정된 조문"을 찾기 쉬워짐(지원 규정 수 52개 불변)
  - 문서레벨 조문 목록과 조문 상세에 각 조문의 최신 이력 힌트 `latest_history`(예: "개정 2025.12.30(공포)", "본조신설 2026.6.30(공포)")를 표시. 종전에는 개별 조문을 하나씩 열어야만 개정 여부가 보였으나, 이제 목록에서 최근 변경 조문을 바로 식별 가능(법령 트랙 한정)
  - 날짜는 공포일(시행일이 아님)이며, 이력 힌트가 없는 조문을 "개정되지 않았다"로 단정하지 않도록 안내를 병기

### 2026. 7. 7. : v0.14.0

- 가지조문(제N조의M — 제7조의2 등 개정으로 신설·추가된 조문) 검색·상세조회 지원(지원 규정 수 52개 불변)
  - 종전에는 "제N조의2" 형태 조문이 도구에 나오지 않았으나, 이제 문서레벨 조문 목록·검색·상세조회에서 확인 가능(최근 개정으로 신설된 조문 조회 개선)

### 2026. 7. 6. : v0.13.1

- 중소기업 기술혁신 촉진법·시행령 현행 정합성 갱신(2026-07-01 개정 발효 반영 — 법률 시행일 2026-05-26 → 2026-07-01·시행령 2026-02-01 → 2026-07-01)
  - 규정 본문은 평소처럼 최신본을 자동으로 조회하며, 이번 갱신은 목록 표시 시행일과 조회 실패 시 대비용 기준 정보를 현행에 맞춘 것(지원 규정 수 52개 불변)

### 2026. 7. 5. : v0.13.0

- 과기부 R&D 규정 1건 추가 지원(혁신도전형 연구개발사업군의 지정 및 분류 기준 등에 관한 고시 / 지원 규정 51 → 52개)
  - 핵심 분류 기준(밀착관리형·공개경쟁형 유형 구분)은 별지 1 수록이라 도구 본문 조회 대상이 아님을 응답에 정직하게 안내(지정 절차·해제·특례 조문은 본문 전문 제공)

### 2026. 7. 1. : v0.12.0

- 산업부 R&D 규정 2건 추가 지원(산업기술혁신사업 운영 지침: 보안관리요령·기술개발 평가관리지침 / 지원 규정 49 → 51개)

### 2026. 6. 30. : v0.11.0

- 과기부 R&D 규정 3건 추가 지원(연구산업진흥법 family: 법·시행령·시행규칙 / 지원 규정 46 → 49개)

### 2026. 6. 30. : v0.10.1

- 조문 본문 정확도 개선: 법령 조문의 '호(號)' 아래 '목(目, 가·나·다…)' 본문을 빠짐없이 수록
  - 예) 기업유형별 연구전담요원 최소 인원처럼 '목'에만 적힌 기준 수치가 이제 조문 본문에 함께 표시됨

### 2026. 6. 29. : v0.10.0

- 과기부 R&D 규정 3건 추가 지원(기업부설연구소등의 연구개발 지원에 관한 법률 family / 지원 규정 43 → 46개)
