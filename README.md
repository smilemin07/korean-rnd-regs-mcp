...
- **지원 범위 = 58개 규정**(혁신법 family·부처별 R&D·핵심 행정규칙). 이 범위 밖 규정에 대한 답변은 일반 지식 기반 안내이니, [국가법령정보센터](https://www.law.go.kr) 등 1차 출처를 함께 확인하세요.
...
| `list_rule_sets`         | 등록된 58개 규정 목록·hierarchy rank·문서 ID·소관부처 조회                                                   |
...
| Prompt                | 용도                                                                                                                                                                                          |
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `review_regulation` | **다층적 규정 검토** — 특정 사례에 대해 58개 규정을 위계 순서로 적용하여, 규정 검토 결과를 근거 조항 인용과 함께 답변. 'korean-rnd-regs-mcp', '규정 검토' keyword를 prompt에 포함 필요 |
