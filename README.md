...
| `search_manual`          | 「국가연구개발혁신법 매뉴얼」 본권 + 별권 3 「제재처분 가이드라인」 + 별권 2 「기술료 제도 매뉴얼」 + 별권 1 「학생인건비통합관리 제도 매뉴얼」 + 「국가연구개발 과제평가 표준지침」(25.12) 통합 검색 — 절 단위 매치(`source`로 자료 구분) + 인쇄쪽 앵커 발췌 + 완성형 인용구 `citation` (v0.38.0 과제평가 표준지침 추가) |
| `get_manual_section`     | 매뉴얼·지침 절 본문 조회(본권 `3-4`형·별권 3 `b3-4-2`형·별권 2 `b2-3-2`형·별권 1 `b1-3-4`형·과제평가 표준지침 `eval-3-2`형 id) — 대형 절은 페이지 경계 분할 청크. 모든 비오류 응답에 규범성 메타(해설 자료·법령 우선)·완성형 인용구·하단 표준 안내 블록 동반 (v0.28.0). 표·산식 구조가 추출 텍스트에 보존되지 않은 절은 완성형 안내 블록 `structure_notice` 동반 (v0.34.0) |
