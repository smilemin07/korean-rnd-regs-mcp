...
> - 자료 종류(깊이): 국가법령정보센터 OpenAPI가 제공하는 법령·행정규칙 중심 + 혁신법 매뉴얼 해설(본권 v0.27.0·별권 3 제재처분 가이드라인 v0.32.0 추가) → 나머지 별권 등 추가 자료로 계속 확대 예정
...
| `search_manual`          | 「국가연구개발혁신법 매뉴얼」 본권 + 별권 3 「제재처분 가이드라인」 해설 통합 검색 — 절 단위 매치(`source`로 자료 구분) + 인쇄쪽 앵커 발췌 + 완성형 인용구 `citation` (v0.32.0 별권 3 추가) |
| `get_manual_section`     | 매뉴얼 절 본문 조회(본권 `3-4`형·별권 3 `b3-4-2`형 id) — 대형 절은 페이지 경계 분할 청크. 모든 비오류 응답에 규범성 메타(해설 자료·법령 우선)·완성형 인용구·하단 표준 안내 블록 동반 (v0.28.0) |

### 혁신법 매뉴얼 해설 (v0.27.0)

「국가연구개발혁신법 매뉴얼」(본권, 26.7판·법령 시행일 2026.6월 기준) 본문 해설부(5개 장 40개 절 + 참고 3건·인쇄 1~332쪽)와 별권 3 「국가연구개발사업 제재처분 가이드라인」(26.7 게시 세트·5개 장 22개 절 + 부록 1건·인쇄 1~89쪽·v0.32.0 추가)을 절 단위로 검색·조회할 수 있습니다.
...
- 수록 범위: 본권 본문 해설부 + 별권 3 「제재처분 가이드라인」 전문(인쇄 1~89쪽). 부록 관련 서식(26.7판부터 별도 PDF로 분리 발간)과 나머지 별권 3종(학생인건비·기술료·연구시설장비)은 미수록입니다.
