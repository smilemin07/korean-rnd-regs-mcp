...
def test_contract_version_0_12_0():
    """v0.16.0: 검색 경로 개정 이력 노출 — search_provision/suggest 결과의 law 조문 매치에 latest_history
    필드 추가(응답 schema 신규 필드) → contract 0.11.0 → 0.12.0 minor bump."""
    from korean_rnd_regs_mcp.provision_id import CONTRACT_VERSION
    assert CONTRACT_VERSION == "0.12.0"


def test_package_version_0_16_0():
    """패키지 버전 0.16.0(major=큰 변화: 가운데 +1·마지막 0 — 검색 경로 개정이력 발견성, contract bump ⟺ major)."""
