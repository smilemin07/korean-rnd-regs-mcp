...
def test_eval_corrupt_data_isolated(fresh_cache, monkeypatch, tmp_path):
    ...
    assert resp["errors"] == []
    assert resp["searched_sources"] == ["main", "b3", "b2", "b1", "b4", "case"]
...
def test_eval_descriptor_position_locked(fresh_cache):
    """v0.39.0 b4·v0.43.0 case append 후에도 eval은 index 3 — rank 4 불변(append-only 잠금)."""
    descs = main_mod._MANUAL_SUPPLEMENTS
    ev = descs[3]
    assert descs[-1]["source_id"] == "case"
