...
**시행일 기준 조회 전환(contract 0.36.0 유지)** — v0.54.0이 "고지"로 알렸던 시행 단계 차이를 **본문 자체에서 바로잡습니다**. 법령(law) 35개 규정의 조문·별표 본문을 공포 합본(`lawService target=law`)이 아니라 **시행일 기준 편집본**(`target=eflaw&efYd=<시행일자>`)에서 가져옵니다. **검색과 상세 조회를 함께** 전환했습니다(분리하면 검색이 옛 조문을 안내하고 상세가 새 조문을 반환하는 모순이 생깁니다). 시행일 기준 조회가 실패하면 **곧바로 종전 방식으로 되돌아가므로**(폴백) 실패해도 오늘보다 나빠지지 않습니다. 패키지 **0.54.0 → 0.55.0**, `contract_version` **0.36.0 유지**(응답 필드·오류 코드·입력 스키마 변경 0 — `warnings` 배열 원소의 조건부 증감뿐). **웹 커넥터 재연결 불요·외부 URL 불변.** 「법령 시행 단계 정합 시리즈」 **P3**.

★기한이 있는 릴리스입니다: **2026-09-11**에 혁신법 시행령의 현행 판본이 288335로 바뀌면서, 종전 방식으로는 그날 시행 중인 본문 대비 **조문 10개**(제45조의2·제48조의2 누락 + 제13·16·17·20·25·26·31·35조 상이)가 틀리게 됩니다(2026-08-25 직접 대조).

### Changed

- **법령 본문 공급원 전환** — 조회 순서는 `resolve`(현행 판본 판정) → 그 판정이 준 문서번호·시행일자 쌍으로 `eflaw` 조회 → 실패 시 `law` 폴백입니다. **시행일자는 판정 결과에서만** 가져옵니다(표시용 값이나 manifest 값을 쓰면 문서번호와 짝이 깨져 빈 응답이 됩니다 — 2026-08-25 실측). 추가 네트워크 조회는 없습니다(판정이 이미 두 값을 함께 반환하므로 35/35 규정에서 그대로 사용 가능함을 실측).
- **이용자 체감 변화** — 공포되었으나 **아직 시행되지 않은 조문은 검색·상세에서 나오지 않습니다**(예: 혁신법 시행령 제35조의2). 반대로 이미 시행됐는데 빠져 있던 조문이 나옵니다(예: 중소기업 기술혁신 촉진법 제27조의4·제27조의5·제28조의2). 시행 전 판본이 필요하면 국가법령정보센터에서 기준일을 지정해 확인하십시오.
- **시행 단계 차이 고지(v0.54.0)의 발화 조건** — 시행일 기준 본문이 정상 제공된 경우에는 **억제**됩니다(그 본문에는 "반영되어 있지 않습니다"가 거짓이므로). 폴백·판정 실패에서는 종전대로 유지됩니다.
- **조문 수 안내 정정** — 중소기업 기술혁신 촉진법 `known_limitations`의 "조문 49"를 **"조문 52"** 로 정정(시행일 기준 실측). 혁신법 시행령 주석의 조문 수도 70(시행일 기준)으로 병기.

### Added

- **폴백 안내 1줄** — 시행일 기준 조회에 실패해 공포 합본을 대신 제공한 경우, 상세 응답 `warnings`에 그 사실과 확인 경로를 알리는 문장 1개가 조건부로 실립니다. 시행 단계 고지가 없는 나머지 31개 법령에서도 열화 상태가 보이게 하는 것이 목적입니다. **판정 실패로 시행일자를 얻지 못해 조회를 시도조차 못 한 경우에는 발화하지 않습니다**(그 상태는 기존 `resolve_fallback_notice`가 전담).
- **조회 경로 집계 로그** — 검색 fan-out 요약 로그에 `stage_eflaw`·`stage_fallback`·`stage_law` 카운터를 추가했습니다(정수만 기록·비밀값 미포함). 조회 경로는 응답에 노출하지 않으므로(노출하면 계약 additive가 됩니다) 운영 중 폴백 잔존을 확인하는 직접 증거입니다.
- **시행일 도래 시 판정 재평가** — 예정 판본의 시행일이 오늘에 도달하면 24시간 판정 캐시 항목을 **실제로 제거**하고 즉시 재판정합니다. 없으면 2026-09-11 전환이 이용자마다 최대 24시간 늦게 반영됩니다.
- **가드 테스트 44종**(`tests/test_stage_eflaw.py`·전체 테스트는 1,010 → 1,055) — 요청 0회 방지·전용 대기 예산·4중 성공 판정·실패 6종 폴백·인증 실패 즉시 전파·서킷 브레이커 개폐·폴백 본문의 24시간 캐시 비오염·조회 경로의 본문 미기록·시행일자 정규화·판정 캐시 실제 제거·고지 억제/폴백 안내/내부 필드 미노출·경로 집계 로그·2026-09-11 경계 전환.

### Fixed

- **폴백 실패 기억이 유효한 성공 캐시를 가리는 회귀**(배포 전 자체 적발·수정) — 시행일 기준 조회 실패
  후의 종전 방식 폴백이 24시간 성공 캐시를 확인하지 않고 네트워크를 타다 실패하면, 그 실패 기억이
  성공 캐시를 가려 종전 경로까지 5분간 오류가 되는 경로가 최종 재검증에서 재현되었습니다. 폴백이
  살아 있는 성공 본문을 먼저 반환하고, 실패 기록 직전 성공을 재확인하도록 고쳤습니다.
- **동시 요청 경합에서 성공과 오류가 공존하는 창**(배포 전 자체 적발·수정) — 한 요청이 시행 본문
  조회에 성공한 직후, 같은 문서를 조회하던 다른 요청이 폴백 본문이나 오류를 반환할 수 있는 창을
  닫았습니다. 폴백 반환·오류 전파 직전에 성공을 재확인하고, 재확인한 본문도 4중 판정을 거칩니다.
- **다단계 시행 법령의 시행일자 오표기** — 시행 단계가 둘인 법령 2건(학술진흥법·국방과학기술혁신 촉진법)에서 종전 조회가 응답 본문에 **이미 지나간 옛 단계의 시행일자**를 싣고 있었습니다(2021-03-23·2024-04-10). 전환으로 정확해집니다.

### Security

- 조회 경로·시행일자 값은 로그에 정수 카운터로만 남기며, 기존 요약 로그의 비밀값 미포함 규약을 그대로 따릅니다.

## [0.54.0] - 2026-08-25

**법령 시행 단계 차이 고지 — 상세 조회 경로 한정(contract 0.36.0 유지)** — 서버가 법령 본문을 가져오는 국가법령정보 API(`lawService target=law`)는 그 공포본의 개정규정을 **시행 단계와 무관하게 전부 합친 텍스트(공포 합본)** 를 돌려줍니다. 그래서 한 법령이 여러 날짜로 나뉘어 시행되는 구간에서는 (가) 이미 시행된 개정이 본문에 없거나 (나) 아직 시행되지 않은 개정이 현행처럼 보일 수 있습니다. 2026-08-23 law 대상 규정 **전건(35건)** 을 `target=eflaw`(시행일 기준 편집본)와 조문·별표 단위로 전부 대조한 결과 **실질 차이 4건**을 확인했고, 그 4개 규정의 **상세 조회 응답에만** 어떤 조문이 어긋났는지 알리는 문장 1개를 추가합니다. 패키지 **0.53.1 → 0.54.0**, `contract_version` **0.36.0 유지**(응답 필드 신설·오류 코드 신설·입력 스키마 변경 없음 — 이미 계약에 있는 `warnings` 배열의 원소가 조건부로 하나 늘어날 뿐). **웹 커넥터 재연결 불요·외부 URL 불변.** 「법령 시행 단계 정합 시리즈」 P2(잠정 고지) — 본문 자체를 시행일 기준으로 바로잡는 것은 P3에서 다룹니다.

### Added

- **시행 단계 차이 고지** — `get_provision_detail`의 문서·조문(JO)·별표(BP) 응답에서 `warnings` 배열의 **첫 원소**로 실립니다. 대상 4규정과 내용:
  - **국가연구개발혁신법** — 2026-06-11 시행된 제2조제5호의2·제2조제10호, 제17조제1항, 제18조제2항, 제32조제1항제5호의 개정이 본문에 **누락**.
  - **국가연구개발혁신법 시행령** — 2026-09-11 시행 예정(제35조의2·제36조·제37조)과 2027-01-01 시행 예정(제16조제2항·제17조제1항·제4항·제5항·제31조·제41조제2항 각 호·제41조제3항·제5항) 내용이 **선반영**.
  - **중소기업 기술혁신 촉진법** — 2026-05-26 시행된 제2조제6호부터 제12호까지의 신설, 제27조의2제3항 삭제, 제27조의3(사업화 금융지원) 신설, 종전 제27조의3의 제27조의4 이동·개정, 제27조의5·제28조의2 신설이 **누락**.
  - **산업기술혁신 촉진법** — 제5조제3항이 인용한 법률명이 종전 명칭(「지방자치분권 및 지역균형발전에 관한 특별법」)으로 남아 있음(현행 「지방자치분권 및 균형성장에 관한 특별법」).
- **감사 도구 수록** — `scripts/audit_law_stage_diff.py`(read-only·로컬 실행 전용·런타임 무관)와 그 순수 함수 가드 테스트, 전수 감사 기준선 2종(`docs/audit_baseline/` — 문면의 근거가 된 2026-08-23분과, 배포 직전 LIVE 재실행으로 같은 결과를 재확인한 2026-08-25분). law 대상 규정마다 같은 파서로 `law`와 `eflaw` 본문을 뽑아 **공백 정규화 후 조문·별표 단위 전문 비교**(full diff)하고, 시행 단계 표에서 "누락 예측·선노출 예측" 플래그를 함께 계산합니다(2026-08-23 감사에서 실측 차이와 4/4 일치). 수집·파싱 오류, 조문/별표 키 충돌, 현행 행 0건·복수는 "차이 없음"으로 읽히지 않도록 판정을 `UNKNOWN`으로 올립니다(fail-closed).

### Changed

- 중소기업 기술혁신 촉진법·산업기술혁신 촉진법의 기존 안내에 적힌 조문 수를 실측값으로 정정했습니다(각각 33 → 49, 49 → 72). 이 두 문장은 상세 조회뿐 아니라 **검색 결과에도 함께 실리는 기존 안내**이므로, 그 두 규정이 검색 결과에 나올 때 표시되는 안내 문구도 같이 바뀝니다(자릿수가 같아 응답 크기·결과 수·순서에는 영향이 없습니다).

### Note

- **고지가 뜨는 조건(판본 결합)**: 문면은 감사 당시의 판본에 결합돼 있어, 그 응답이 실제로 조회한 판본이 **감사 판본과 정확히 같을 때만** 구체 내용이 나옵니다. 다른 판본이 서빙되면(시행예정일 도래 등) 구체 문면 대신 "이 판본은 감사되지 않았다"는 일반 주의 문장으로 바뀝니다. 상세 조회는 매 호출 최신 판본을 다시 확인하고 그 결과가 이용자별로 최대 24시간 캐시되므로, 전환일에는 같은 시각에도 이용자마다 다른 판본이 서빙될 수 있습니다 — 문면을 한 시점에 한 번 갈아 끼우는 방식으로는 모든 응답을 정확하게 만들 수 없어 이렇게 설계했습니다.
- **시간에 따라 어긋날 수 있는 부분**: 판본 결합은 "어느 본문인가"를 맞출 뿐 "오늘이 며칠인가"를 반영하지 않습니다. 예를 들어 2026-09-11이 지난 뒤에도 캐시로 같은 판본을 받는 이용자에게는 "2026-09-11 시행 예정"이라는 문면이 그대로 나갈 수 있습니다(그때는 이미 시행된 뒤이고, 남는 차이는 2027-01-01 시행분입니다). 문면이 "감사일 현재"임을 밝혀 오독을 줄이지만, 근본 해소는 본문을 시행일 기준으로 조회하는 P3와 2026-09-11 재감사에서 이뤄집니다.
- **보증하지 않는 것**: 이 고지는 2026-08-23 감사일 기준으로 확인된 차이만 알립니다. 열거되지 않은 부분이나 이후 판본의 정합성을 보증하지 않으며, 고지가 없다고 해서 그 규정이 시행 단계상 정확하다는 뜻도 아닙니다. 표시된 조문의 시행일 기준 본문은 국가법령정보센터(law.go.kr)에서 기준일을 지정해 확인하십시오.
- **검색 응답에는 붙이지 않습니다.** `search_provision`은 결과 1건마다 안내를 반복 직렬화하고 전체 응답에 길이 예산이 있어, 문면을 넣으면 뒤쪽 결과가 잘려 **검색 결과 수가 줄어듭니다**(로컬 실측 최대 3건 감소). 검색 품질을 깎지 않기 위해 상세 조회 경로로 한정했습니다. `list_rule_sets`·`suggest_review_sources`에도 노출되지 않습니다. 정확히 하자면 이번 릴리스에서 **검색 응답이 완전히 동일한 것은 아닙니다** — 새 고지는 붙지 않지만, 위 Changed의 조문 수 정정 2건은 검색 결과에 실리는 기존 안내 문구의 값을 바꿉니다. 바뀌지 않는 것은 검색 결과 수·순서·랭킹과 응답 길이입니다(8개 대표 질의 구·신 짝 비교에서 결과 수 델타 0 실측).
- 사용자별 변경: 웹 커넥터·PyPI·uvx·플러그인 이용자 모두 (가) 위 4규정의 상세 조회에서 주의 문장 1개가 warnings 맨 앞에 추가되고 (나) 두 규정의 기존 안내 문구에 적힌 조문 수가 실측값으로 바뀌는 것, 이 두 가지가 전부입니다. 도구 목록·입력 파라미터·검색 결과 수와 순서·매뉴얼 트랙은 무변입니다.
- 이어지는 패키지: P3(법령 본문을 시행일 기준으로 조회하도록 정확화 — 2026-09-11 시행분 도래 전 필요) → P4(2026-09-11 시행분 manifest 현행화·재감사).
...
- **`Dockerfile`의 `pip install` 한 줄에 `requests==2.34.2`·`urllib3==2.7.0` 핀 추가**(기존 `fastmcp==3.4.2`·`uvicorn==0.48.0`·`python-multipart==0.0.30` 핀과 동일 방식). 값은 2026-08-23 NAS 라이브 v0.52.0 컨테이너 실측과 동일합니다. 새 이미지는 배포 게이트에서 `pip freeze`를 라이브와 전수 대조해 차이가 0건일 때만 라이브와 동일한 구성으로 판정하고 교체합니다(차이가 있으면 교체하지 않음).
  - 게이트 실측(2026-08-23): 1차 후보 이미지에서 `cyclopts`(fastmcp CLI 전용·본 서버 런타임은 import하지 않음) 1건이 4.23.1→4.23.2로 드리프트해 규칙대로 교체를 중단했고, 라이브 실측값 `cyclopts==4.23.1` 핀을 추가해 재빌드한 뒤 차이 0건을 다시 확인했습니다. 최종 핀은 6종입니다.
  - 근거: v0.48.0의 LIVE 연결 재사용(thread-local keep-alive Session)은 두 라이브러리의 기본값 의미론(`trust_env`·adapter retry·pool sizing)에 기대어 안전성을 증명했는데, 이미지를 다시 빌드할 때마다 두 패키지가 최신으로 풀려 라이브러리가 기본값을 바꿔도 어떤 게이트도 잡지 못하는 상태였습니다(v0.49.0 재빌드 시 간접 의존성 8건의 버전이 달라졌던 배포 기록이 근거).

### Note

- 사용자별 변경: 웹 커넥터 이용자 **0** · PyPI·uvx·플러그인(stdio) 이용자 **0**(이 핀은 Docker 이미지에만 적용되며 `pyproject.toml`의 의존성 범위는 그대로) · 자체 호스팅 운영자 — 사용자 노출 기능 변경 없음(재빌드 시 두 패키지 버전이 고정될 뿐·이미지 재빌드·검증·교체 작업은 운영자 몫).
- 이어지는 패키지: P2(시행 단계 오차 잠정 고지 + law↔eflaw 전수 감사) → P3(법령 본문 시행 단계 정확화) → P4(2026-09-11 시행분 manifest 현행화).

## [0.52.0] - 2026-08-22

**HTTP 수신 인터페이스 노출면 축소 — `FASTMCP_HOST` 존중(contract 0.36.0 유지)** — `--http` 모드의 바인딩 주소를 운영자가 환경변수로 지정할 수 있게 합니다. 지금까지는 코드가 `0.0.0.0`(모든 네트워크 인터페이스)을 고정 전달해 fastmcp의 표준 환경변수 `FASTMCP_HOST`가 무시됐고, 그 결과 host 네트워크로 운영하는 자체 호스팅(예: NAS + Cloudflare Tunnel)에서는 터널을 거치지 않고도 같은 LAN에서 포트에 직접 닿을 수 있었습니다(실측 2026-08-21·키 없는 요청은 기존 가드로 차단되므로 데이터 유출 경로는 아니었음). 패키지 **0.51.0 → 0.52.0**, `contract_version` **0.36.0 유지**(도구 응답·입력 스키마 무변). **웹 커넥터 재연결 불요·외부 URL 불변.**

### Changed

- **`--http` 바인딩 주소가 `FASTMCP_HOST` 환경변수를 따릅니다.** 값은 앞뒤 공백을 제거해 그대로 사용하며, **미설정·빈값·공백이면 종전과 같은 `0.0.0.0`** 입니다. 같은 호스트의 터널·리버스 프록시 뒤에서 운영한다면 `FASTMCP_HOST=127.0.0.1`로 loopback에 한정하는 것을 권장합니다.
  - ★fastmcp 자체 기본값(`127.0.0.1`)과 다릅니다. 기본값을 loopback으로 바꾸면 Docker bridge 포트매핑(`-p 8080:8080`)이나 LAN 직접 접속으로 운영 중인 자체 호스팅이 업그레이드만으로 끊기므로, 기존 동작을 보존하고 명시 설정으로만 좁히도록 했습니다(기본값 변경·방화벽 단독 대응은 계획 검토에서 기각).
  - 빈값·공백 폴백은 부팅 생존용 최소 가드입니다(실측: uvicorn은 `''`를 전체 바인딩으로, `' '`·`'127.0.0.1:8080'`은 `SystemExit(1)`로 처리 — 재시작 정책과 만나면 재시작 루프). 그 외 형식 검증은 의도적으로 하지 않습니다(오설정은 즉시 실패가 정답).
- `--http` 도움말(`--help`)에 바인딩 주소 안내 1줄 추가.

### Note

- 사용자별 변경: 웹 커넥터 이용자 **0** · PyPI·플러그인(stdio) 이용자 **0** · 자체 호스팅 기본 동작 **0** · 운영자는 compose `environment`에 `FASTMCP_HOST=127.0.0.1` 1줄.
- `0.0.0.0` 고정 전달의 출처는 폐기된 Fly.dev 배포(커밋 8e07859)였습니다.
- ★이 설정이 줄이는 것은 **커널 네트워크 인터페이스 노출**(LAN 직접 접근)입니다. 같은 호스트에서 loopback으로 대신 접속해 주는 프로세스를 통한 도달은 바인딩 주소로 막을 수 없습니다 — 실측(2026-08-22): Synology Tailscale 패키지는 `--tun=userspace-networking`로 동작해 tailnet 요청을 `127.0.0.1`로 프록시하므로, `FASTMCP_HOST=127.0.0.1` 후에도 tailnet 구성원(운영자 인증 기기)은 포트에 닿습니다. 이 경로는 본 릴리스 범위 밖이며 Tailscale ACL 등 운영 정책의 후속 과제로 기록합니다. 배포 게이트도 이에 맞춰 "Tailscale 직접 호출 거부"(잘못된 대리 지표)가 아니라 "LAN IP 직접 호출 거부 + loopback 단독 LISTEN + 터널 정상 + 무키 406"으로 검증했습니다.

## [0.51.0] - 2026-08-21

**manifest 4건 현행화(2026-08-20 시행분) + 상세 조회 미시행 시행일 고지(contract 0.36.0)** — 2026-08-20 시행 도래로 국가연구개발혁신법 family 3건과 국토교통과학기술 육성법 시행령의 등록 스냅샷을 현행 판본으로 갱신하고, 조회된 원문 자료가 아직 시행되지 않은 판본일 때 응답이 스스로 경고하는 안내 필드 1종을 추가합니다. 특히 구 혁신법 스냅샷(283849)은 단순 구판이 아니라 **2026-09-11 시행 예정 판본의 본문을 반환**하고 있어(현행과 제2·17·18·32조 상이), 검색 실패 시 최후 경로(fallback)가 미시행 조문을 현행처럼 제공할 수 있는 상태였습니다. 패키지 **0.50.0 → 0.51.0**, `contract_version` **0.35.0 → 0.36.0**(§5.42 — 조건부 additive 필드 1종). **입력 스키마 무변 → 웹 커넥터 재연결 불요.**

### Changed

- **manifest 4건 현행화**(2026-08-21 lawSearch 현행 행 재프로브·66규정 전수 감사 = 일치 62·CHANGED 4·실패 0): `innovation_act` 283849 → **283413** / `innovation_decree` 288335 → **288773**(조문 69→71·★별표5의2 「연구보안 전담기관에 대한 행정처분의 세부기준」 신설 = `BP000502`·별표2·7 대용량 포인터 유지) / `innovation_rule` 286879 → **289003**(서식 10→12종·BP 미지원 동일) / `sector_kt_decree` 264735 → **288777**. effective_date 4건 전부 **2026-08-20**.

### Added

- **`fetched_detail_effective_date_notice`(law 한정·조건부)**: `get_provision_detail`이 가져온 국가법령정보 상세 자료 자체의 시행일자가 오늘(KST)보다 미래이면 완성형 안내 1문장을 부착합니다 — "조회된 국가법령정보 상세 자료의 시행일자가 {날짜}로 오늘보다 미래입니다. …". 검색 행 기준의 기존 방어(자동 최신판 선택·현행성 감사)가 전부 통과한 채 미시행 본문이 제공될 수 있는 사례가 2026-06-12~08-19 혁신법에서 69일간 실측되어, 그 창을 응답 자체가 고지하도록 했습니다(추가 네트워크 호출 0·날짜 형식 이상 시 미발화). 발화해도 요청한 특정 조문이 미시행이라는 단정은 아니고, 부재가 현행성 보증도 아니며, 검색 발췌 경로는 보호 범위 밖입니다(§5.42 비보증 3항).
- **배포 전 감사 도구 `scripts/audit_manifest_effective_dates.py`**: 등록 스냅샷 ID를 검색을 거치지 않고 상세 조회하여 시행일자가 미래이면 FAIL로 판정(PASS/FAIL/UNKNOWN 3분류·UNKNOWN은 통과 아님). 위 69일 사례의 재발을 배포 시점에 구조적으로 차단합니다(런타임 무관·저장소 도구).

### Note

- 구 문서 ID(283849·288335·286879·264735)를 담은 provision_id를 이전 대화에서 재사용하면 `not_found`가 됩니다 — 오류 안내대로 재검색하면 현행 ID로 복구됩니다(ID 변경 릴리스의 기지 성질).
- 혁신법은 **2026-09-11 시행 예정 개정분**이 추가로 공포되어 있습니다(정부납부기술료 정의 신설 등). 해당 시행일 도래 시 현행 판본이 다시 바뀔 예정이며, 재감사 후 후속 데이터 릴리스로 반영합니다.

## [0.50.0] - 2026-08-20

**규정 탐색·조회 실패 복구 안내 정비(contract 0.35.0)** — 검색·조회가 실패하는 3개 지점에 호스트 AI가 따라갈 복구 경로를 안내 문면으로 심습니다(2026-08-19 라이브 집중 테스트에서 관측된 마찰 해소). 런타임 로직·데이터·응답 필드 구조·오류 코드는 전부 무변이고 문면 2건과 프롬프트 표면 2곳만 바뀝니다. 패키지 **0.49.0 → 0.50.0**, `contract_version` **0.34.0 → 0.35.0**(§5.41 — 계약 명시 필드의 문면 값 변경). **입력 스키마 무변 → 웹 커넥터 재연결 불요.**

### Changed

- **개정 예정 고지(`upcoming_revision`) 문면 정비**: 후반부를 "시행 전 새 판본 본문은 본 도구로 조회할 수 없으니 국가법령정보센터(law.go.kr)에서 확인하고 시행일부터 본 도구로 다시 조회하십시오."로 교체 — 개정 예정 신판의 본문을 도구로 미리 조회할 수 없다는 한계와 확인·복귀 경로를 고지 자체에 명시합니다(접두 "개정 예정: …"은 불변).
- **규정 상세 조회 실패(`not_found`) 문면 정비**: "manifest에 … 항목 없음"(내부 용어) → "지원 규정 목록에 {ID} 항목 없음 — 규정 개정으로 문서 ID가 바뀐 구 ID이거나 미지원 규정일 수 있으니, 질문 핵심 용어로 search_provision을 재호출해 현행 provision_id를 확인하십시오". 구 ID 재사용 시의 재검색 복구 행동(v0.49.0 배포 후 실증)을 오류 문면 자체에 내장합니다.
- **검색 용어 드리프트 재검색 힌트(프롬프트-only)**: 서버 instructions와 `search_provision` 설명에 1개 지시 추가 — 법령 개정으로 조문 용어가 바뀐 경우(예: 제재처분 '이의신청'은 2026년 2월 개정으로 혁신법 제33조에서 '재검토 요청'으로 전환) 구 용어 검색이 미도달이면 현행 용어로 재검색하되, 구·현행 용어를 한 검색어에 함께 넣지 않도록(토큰 AND 매칭이라 0건) 안내합니다. 검색 알고리즘·동의어 매핑은 변경하지 않습니다.

### Note

- 세 변경 모두 "탐색·조회 실패 시 복구 안내"라는 단일 의도이며, 규정 데이터·검색 매칭/랭킹 알고리즘은 종전과 동일합니다. 개정 예정 고지 문면이 47자 길어져, 향후 개정 예정 판본이 발화되는 시점에는 응답 크기 예산의 경계 사례에서 검색 결과 절단·본문 강등 지점이 그만큼 보수적으로 당겨질 수 있습니다(현재 지원 66개 규정에 개정 예정 판본 없음 — 발화 0).

## [0.49.0] - 2026-08-20

**rnd_info_processing manifest 현행화 — 데이터-only(contract 0.34.0 유지)** — 「국가연구개발정보처리기준」의 등록 스냅샷을 2026-08-20 시행 개정본(과기정통부고시 제2026-47호 일부개정)으로 갱신합니다. `rule_sets.yaml` 2값(api_doc_id 2100000195842 → 2100000283100·effective_date 2021-01-01 → 2026-08-20)만 변경하는 데이터-only 릴리스로 코드 실행 경로 변경이 없습니다. 시행일 도래 후 실시간 resolve는 manifest와 무관하게 신판을 자동 선택하지만, resolve 실패 폴백·`list_rule_sets` 표시·개정 안내(is_updated) 정확성을 위해 스냅샷 동기화가 필요합니다. 패키지 **0.48.0 → 0.49.0**, `contract_version` **0.34.0 유지**(§5.40). **입력 스키마 무변 → 웹 커넥터 재연결 불요.**

### Changed

- **`rnd_info_processing` 등록 스냅샷 현행화**: 구판 제2020-102호(2021-01-01 시행·2100000195842) → 신판 제2026-47호(2026-08-20 시행·2100000283100). 신판 LIVE 실측(2026-08-13) = 시행일 20260820 유지·조문 25·별표 2(0000·0002) 구조 동일·별표 0000 본문 39,259자 → 178,275자 확대(기존 oversized_pointer·annex_chunk 경로가 처리).
- README 지원 규정 표의 해당 시행일 표기 동기화(2021-01-01 → 2026-08-20).

### Note

- 구 문서 ID(2100000195842)를 담은 provision_id를 이전 대화에서 재사용하면 `not_found`가 됩니다 — 재검색으로 새 provision_id를 받으십시오(ID 변경 릴리스의 기지 성질).
- **배포 시점 제약(이행 완료)**: 본 버전은 2026-08-20 시행 도래 후 fresh client 재프로브 PASS(5/5)·LIVE acceptance 전건 PASS를 확인하고 당일 배포했습니다(도래 전 배포 시 개정 안내 역방향 발화·폴백 시 미시행 본문 제공 위험 — §5.40).
...
- **대형 연구개발 사업계획검토 운영에 관한 규정** (`large_rnd_plan_review` — 과기정통부고시 제2026-15호·2026-03-26 제정·시행·admrul 2100000276390): 대형 연구개발사업 사업계획검토의 운영 절차(검토 수행기관·점검항목 등) — 조문 15. 부속 별지 2건(사업계획서의 제출·사업계획검토의 점검항목)은 별표구분='별지'라 BP(별표) 조회 미지원.
- **구축형 연구개발사업 심사 운용지침** (`build_type_rnd_screening` — 과기정통부고시 제2026-41호·2026-05-11 제정·시행·admrul 2100000278982): 구축형 연구개발사업의 심사(사업추진심사 등) 운용 — 조문 42. 부속 서식 1건(심사 요구서·대용량)은 별표구분='서식'이라 BP 조회 미지원. **부칙 제2조 적용례**: 제17조제2항제2호 각 목은 2026년 11월 이후 사업추진심사를 요구하는 사업부터 적용 — 일부 조항의 적용 시점이며 규정 전체는 2026-05-11부터 시행 중입니다(known_limitations에 명시).

### Changed

- 지원 규정 카운트 3표면 64 → 66 갱신: 최상위 서버 instructions·`review_regulation` 검토 템플릿(적용 범위 Tier 2 공통 행정규칙 목록에 2제목 추가)·README(임베드 프롬프트 byte-sync·지원 규정 표 2행 추가).

### Unchanged (명시)

- 응답 필드·shape·오류코드·검색/랭킹/fallback 알고리즘·입력 스키마·매뉴얼 트랙 7소스 전부 무접촉. 기존 64규정의 manifest 항목 byte 불변. `rnd_info_processing` 현행화(2026-08-20 시행 도래 후 별건)는 본 릴리스에 포함하지 않습니다.

## [0.46.0] - 2026-08-08

**국토교통 R&D 맥락 사례집 라우팅 보강 — 프롬프트-only(contract 0.34.0 유지)** — "국토부 R&D 연구비 집행"·"국토교통부 연구개발비 사용" 같은 부처명이 든 질의에서 「국가 R&D 연구비 부적정집행 사례집」(KAIA)이 검색에 잡히지 않는 결손을 해소합니다. 원인은 사례집 본문에 부처명 표기가 거의 없어('국토교통' 1건·'국토부' 0건 실측) 부처명 토큰이 든 AND 검색이 0건이 되는 것으로, 데이터·검색 알고리즘의 결함이 아니라 검색어 구성의 문제입니다. 이번 버전은 AI에게 ①국토교통부(국토부) 소관 과제의 연구비 집행·정산·부적정집행 질의에는 사례집을 우선 확인 대상에 포함하고 ②이때 검색어에 부처명을 넣지 말고 '부적정집행'·비목명·'정산' 같은 주제어를 쓰도록 지시합니다(지시 없이 "함께 확인하라"만 넣으면 부처명이 그대로 전달돼 여전히 0건). ③아울러 사례집의 사례는 국가 R&D 전반에서 수집된 자료이므로(사례집 인사말 서술 기준) 국토교통부 전용으로 한정하지 않도록 범위 사실을 함께 안내합니다. 패키지 **0.45.0 → 0.46.0**, `contract_version` **0.34.0 유지**(§5.37 — 응답 필드·입력 스키마·검색/랭킹 무변). **웹 커넥터 재연결(삭제·재등록) 불요** — 변경된 지시문은 새 대화(새 세션 초기화)부터 자동 반영되며 진행 중 대화에는 반영되지 않습니다(도구 설명은 커넥터의 "도구 목록 새로 고침"으로도 반영 가능).

### Changed

- **프롬프트 3표면에 라우팅·검색어 구성 지시 추가**: 최상위 서버 instructions(`_SERVER_INSTRUCTIONS`) · `search_manual` 도구 설명 · `review_regulation` 검토 템플릿(README 임베드 동기). `get_manual_section` 도구 설명은 제외 — 절 id를 확보한 뒤 호출하는 도구라 최초 라우팅에 무효하기 때문입니다.

### Unchanged (명시)

- 응답 필드·shape·오류코드·검색/랭킹/fallback 알고리즘·입력 스키마·규정 트랙·매뉴얼 데이터 7종 전부 무접촉(코드 실행 경로 변경 0). 데이터 축 해법(`law_priority_extra` 추가 등)은 기존 문자열 byte 불변 정책 위반 + 검색 미도달 시 meta도 미노출이라 결손을 원리적으로 못 고쳐 계획 단계에서 기각됐습니다.

## [0.45.0] - 2026-08-08

**규범성 안내 귀속 교정 — 지정 출처 명시(contract 0.34.0)** — v0.44.0 배포 후 라이브 eval에서 응답 구조 언급("시스템 메타데이터…")은 6/6 차단됐지만, AI가 서버 제공 안내(`law_priority_note`)를 소개하며 **"원문 자체에 이렇게 명시되어 있습니다"처럼 자료 원문·발간처의 문장으로 오귀속하는 현상은 잔존**했습니다(추론 강도를 높여도 4관측 재현). 원인은 AI가 응답의 자료 설명 블록에 든 '자료에 관한 서버 안내'를 그 자료의 내용으로 취급하는 데 있어, 금지(부정문)만으로는 부족하고 **어떻게 귀속할지 지정하는 긍정 대안**이 필요하다고 진단했습니다. 이번 버전은 v0.44.0에서 신설한 인접 지시(`law_priority_note_note`)의 **문면만 교체**해, 안내를 소개할 때 반드시 **"korean-rnd-regs-mcp에서 제공하는 정보에 따르면,"**으로 시작해 출처를 밝히도록 지시합니다. `contract_version` **0.33.0 → 0.34.0**(§5.36), 패키지 **0.44.0 → 0.45.0**. **입력 스키마 무변 → 웹 커넥터 재연결 불요**(도구 설명도 무변 — 도구 목록 새로 고침 불요).

### Changed

- **`law_priority_note_note` 문면 교체**(유일한 의도 변경): ①이 안내는 자료 원문이 아니라 korean-rnd-regs-mcp가 제공하는 것임을 명시 ②소개 시 자료·발간처가 밝혔다고 서술하거나 인용하는 것을 금지(직접 인용뿐 아니라 의역 오귀속 포함) ③반드시 "korean-rnd-regs-mcp에서 제공하는 정보에 따르면,"으로 시작해 귀속 ④"시스템 메타데이터"·"내부 필드" 등 전달 방식·응답 구조 언급 금지(v0.44.0 문장 유지). 서버 이름을 밝히는 것은 정보 작성 주체의 신원이므로, v0.44.0이 차단한 내부 구조 노출(전달 메커니즘 지칭)과 다릅니다.

### Unchanged (명시)

- 이 변경은 additive가 아니라 **기존 v0.44.0 필드의 값 변경**입니다. 의도된 도메인 변경은 `law_priority_note_note` 값 하나뿐이며, 그 외 v0.44.0 이전의 모든 응답 필드와 `law_priority_note` 본문은 버전 식별자 — `contract_version`(오류 응답에 실리는 것 포함)과 패키지 버전 표기(`health.version` 등) — 를 제외하고 byte 불변입니다. 새 문면은 직렬화 기준 3자 짧아 응답 크기 예산·양보 백스톱(v0.44.0)의 판정도 그대로입니다(예산 경계 청크 실측 불변). 규정 트랙·매뉴얼 데이터 7종·검색·입력 스키마 전부 무접촉입니다.

## [0.44.0] - 2026-08-08

**규범성 안내의 표시 귀속 교정 — 인접 지시 `law_priority_note_note` 신설(contract 0.33.0)** — v0.43.0 배포 후 라이브 eval에서 AI(claude.ai)가 매뉴얼 응답의 규범성 안내(`manual_meta.law_priority_note` — 자료 성격·법령 우선 안내)를 정확히 인용하면서 **"시스템 메타데이터에 명시된 안내입니다"라고 내부 필드의 존재 방식을 사용자에게 노출**하고, 일부를 "매뉴얼 자체에서도 경고하고 있습니다"라고 발간처 문장으로 오귀속하는 사례를 확인했습니다(P3). 안내 내용의 전달 자체는 정확·유익했으므로 전달은 유지하고 **소개 방식만 교정**합니다. `contract_version` **0.32.0 → 0.33.0**(§5.35), 패키지 **0.43.0 → 0.44.0**. **입력 스키마 무변 → 웹 커넥터 재연결 불요.** 규정·매뉴얼 데이터와 기존 응답 필드는 전부 byte 불변입니다.

### Added

- **`law_priority_note_note`**: 두 매뉴얼 도구(`search_manual`·`get_manual_section`)의 모든 비오류 응답 `manual_meta`에서 `law_priority_note` **바로 뒤 키**로 추가되는 상수 안내(전 소스 공통·단독/혼합 블록 동일). AI에게 ①안내 내용을 답변 판단에 반영하고 필요하면 자료의 성격에 관한 **사실로 자연스럽게 서술**할 것 ②안내문 자체를 자료 원문·발간처의 문장이나 별도 출처처럼 인용하지 말 것 ③"시스템 메타데이터"·"내부 필드" 등 **전달 방식·응답 구조를 사용자에게 언급하지 말 것**을 지시합니다. 인접 지시 필드(`*_note`)는 누적 관측에서 사용자 답변에 노출된 사례가 없고(v0.41.0 `standard_footer_note` 등 4회), 인접 배치만으로 호스트 행동이 바뀐 선례(v0.41.0)를 따른 설계입니다.

- **예산 경합 시 신규 안내가 먼저 양보됩니다**: 응답 크기 예산 경계에 있는 일부 응답(대형 절의 분할 청크·검색 절단 직전 응답)에서는 새 안내를 싣는 대신 기존 내용을 지킵니다 — 표·산식 구조 안내(`structure_notice`)와 검색 결과 개수는 v0.43.0과 동일하게 유지되고, 그 응답에서만 새 안내가 생략됩니다(실측: 경계 청크 2건에서 양보 발동·본문 수록 등급 변화 0·검색 결과 수 변화 0).

### Unchanged (명시)

- `law_priority_note` 문면·`notice`·`standard_footer`·`citation` 등 기존 필드는 `contract_version`(0.32.0 → 0.33.0으로 갱신되는 계약 선언 값)을 제외하고 전부 byte 불변이며, 오류 응답(`manual_meta` 미포함)은 무영향입니다. 완성형 블록 승격안(안내 875자의 답변 verbatim 복사를 유발)과 관측되지 않은 다른 서술 필드로의 예방적 확대는 채택하지 않았습니다.

## [0.43.0] - 2026-08-07

**「국가 R&D 연구비 부적정집행 사례집」(KAIA·25.5) 수록(contract 0.32.0)** — 국토교통과학기술진흥원(KAIA)이 발간한 연구비 사례집(2025.05)을 매뉴얼 트랙 7번째 소스로 수록합니다. 비목별 부적정집행 사례 105건(인건비·학생인건비·연구시설·장비비·연구재료비·연구활동비·연구수당·위탁연구개발비·국제공동연구개발비·간접비·기타)과 FAQ 18문항, 연구비 사용 전 유의사항, 상시·연차점검/정산 절차 안내를 `search_manual`·`get_manual_section`으로 검색·조회할 수 있습니다. `contract_version` **0.31.0 → 0.32.0**(§5.34), 패키지 **0.42.0 → 0.43.0**. **입력 스키마 무변 → 웹 커넥터 재연결 불요.** 기존 매뉴얼 데이터 6종과 규정 트랙은 무접촉이며, 기존 소스 검색 결과 사이의 상대 순서는 보존됩니다(사례집 매치가 관련 질의의 결과 목록에 새로 삽입될 수 있습니다).

### Added

- **`manual_case.json`**: 13개 단위·52,921자(Ⅰ 들어가기전에 `case-1-1` · Ⅱ 비목별 사례 `case-2-1`~`case-2-10` · Ⅲ FAQ `case-3-1` · Ⅳ 상시·연차점검/정산 절차 `case-4-1`). 원본이 좌우 2쪽 스프레드 + 쪽 내부 2열 박스 배치라 신규 추출 기법(반쪽 분할 + 동적 열 감지 정렬)을 도입했고, 정합성은 비목별 사례 번호 1~N 연속(105건)·FAQ Q1~Q18 연속·전체 글자 수 장부 검증으로 잠갔습니다.
- **독립 로더·오류 격리**: `manual_case.json` 로드 실패는 사례집 조회에만 영향을 주며(오류 코드 `manual_case_unavailable`), 본권·별권·과제평가 표준지침·규정 도구는 정상 동작합니다(검색에서는 `source_warnings`로 알림). `search_manual` 응답의 `source` 값에 `"case"`가 추가됩니다.
- **규범성·범위 안내**: 이 자료는 교육·참고용 사례집이며 법령·행정규칙이나 개별 사안에 대한 정산 판정·제재처분 결정이 아니라는 안내(`manual_meta`·하단 표준 안내)와, Ⅳ장의 점검·정산 절차(Ez-baro·위탁정산기관·일정)는 KAIA 국토교통R&D 프로세스 기준이라 타 부처 과제는 해당 전문기관 확인이 필요하다는 사실 라벨, 구체 금액·비율·조문은 「국가연구개발사업 연구개발비 사용 기준」 현행 원문으로 교차 확인하라는 안내를 응답에 동반합니다. Ⅳ장 절차 도식과 8쪽 불인정 기준표(좌우 배치)는 텍스트 추출에서 배치가 보존되지 않아 해당 절에 `structure_notice` 완성형 안내가 붙습니다. 사례집 단독 인용 답변의 하단 안내(2번째 줄)는 원문 확인처를 KISTEP이 아닌 KAIA 홈페이지로 안내합니다(기존 자료의 안내는 변하지 않습니다).

### Deferred

- 함께 요청된 「연구자를 위한 국가연구개발사업 제재제도 설명서」(과학기술정보통신부·KISTEP, 2025.2)는 **수록을 보류**했습니다. PDF의 모든 글자가 벡터 외곽선으로 변환되어 텍스트 레이어가 없고(추출 원문 부재), OCR·전사 수록은 "원문 그대로" 원칙과 충돌하며, 내용이 이미 수록된 「국가연구개발사업 제재처분 가이드라인」(별권 3)과 혁신법 시행령 별표 6·7의 연구자용 요약이기 때문입니다(설명서 자체가 별권 3 참조를 안내).

## [0.42.0] - 2026-08-07

**후보 추천 키워드 품질 보강 — 꼬리 요청 절 필터(contract 0.31.0 유지)** — `suggest_review_sources`에 `keywords`를 함께 넘기지 않으면 서버가 질문 표면에서 키워드를 규칙 추출하는데(fallback), 이때 "…검토해야 할 규정 후보와 검토 순서"처럼 **질문 끝의 요청 형식**을 이루는 단어가 그대로 검색어가 되어 무관 조문이 후보 상위를 점유하는 문제를 확인했습니다(2026-08-07 결정론 재현: 후보 1~3위가 "제재처분의 절차 및 재검토 요청 등"·"규제의 재검토" 2건으로 전부 '검토' 단독 매칭). 원인은 코퍼스 광역어 '검토'(258건)가 제목 우선 랭킹을 타고 올라온 것이며, 랭킹 자체는 설계대로 동작했습니다. 패키지 **0.41.0 → 0.42.0**, `contract_version`은 **0.31.0 유지**(§5.33 — 응답 필드·shape·오류 코드·입력 스키마 불변). **웹 커넥터 재연결 불요.** 규정·매뉴얼 데이터, `search_provision`, 랭킹, 상한은 무접촉입니다.

### Changed

- **질문 끝의 요청 절만 잘라냅니다**: 질문 끝에서부터 앞으로 훑으며 요청 형식어(후보·순서·추천·목록·규정·조문·법령)와 요청 서술형("검토해야"·"추천해"·"안내해줘" 등)을 제외하고, **실질적인 내용어를 만나면 멈춥니다**. 중간에 "하는"·"알려달라" 같은 문법 표현이 끼어도 판정이 이어집니다. 관측 질문에서는 키워드가 8개에서 3개(연구개발비·비목·전용할)로 줄고, 무관 조문이 상위에서 사라집니다.
- **같은 단어라도 조문 용어로 쓰이면 그대로 둡니다**: "후보"·"추천"·"목록"·"법령"·"규정"은 실제 조문 제목이기도 하므로(예: "산업기술혁신평가위원 후보단", "평가위원 추천 기준", "중장기 기술확보 목록", "다른 법령과의 관계") 무조건 지우지 않습니다. "평가위원 후보 추천 기준을 알려줘"처럼 뒤에 내용어가 있으면 전부 보존됩니다. "재검토 요청 절차"·"사전 검토 규정"·"중복성 검토"·"위원회가 검토해야 하는 사항"의 검토도 보존됩니다.
- **의문형·요청 표면어만 검색어에서 제외**: 무엇인가·있는가·되는가·하는가·이루어지는가·하는지·달라·알려줘 등 11종입니다. 이 중 다수는 64개 규정 전체에서 매칭이 0건이라 후보 손실 없이 키워드당 1회씩 발생하던 전 규정 조회를 줄입니다. **"승인을 받아야"·"심의를 거쳐야" 같은 의무 표현과 결과·확인·통보·통지 등 법적 의미가 있는 단어는 제외하지 않습니다.**
- **용언 어미 정규화는 도입하지 않았습니다**(의도된 제외): 어미를 잘라내면 "참여제한"이 "참여제"로 훼손되고, 어간이 광역어인 경우 오히려 정밀도가 떨어집니다(실측: "실시해야" 13건이 정확한 조문을 직격하는 반면 "실시"는 422건, "전용"은 17건 중 다수가 다른 뜻의 專用). 별도 검증 과제로 분리했습니다.
- 요청 형식어만 있는 질문("검토 순서와 규정 후보 목록 추천")은 키워드가 비어 기존 무키워드 안내 응답으로 넘어가며, 이때 전 규정 조회를 시작하지 않습니다. 응답 형식과 하단 표준 안내는 종전과 같습니다.

### Known issues

- 호스트가 노이즈가 섞인 `keywords`를 직접 제공하면서 후보가 1건이라도 나오는 경우에는 fallback이 발동하지 않으므로 이번 변경의 적용을 받지 않습니다(제공 키워드로 결과가 0건이면 보강 경로에서 새 규칙이 적용됩니다).
- 요청 절과 법적 서술의 표면형이 같은 문장("전문기관이 연구자에게 안내해야 할 규정")은 요청 절로 보아 끝부분을 제외합니다. 형태만으로는 구분할 수 없는 경우이며, 앞쪽의 실질 단어(전문기관·연구자)로 관련 조문에 도달합니다.
- ChatGPT의 GPT-5.5 모델에서 응답 뒤쪽에 추가된 안내 필드가 재구성에서 누락되어 하단 표준 안내가 표시되지 않는 사례를 관측했습니다(같은 커넥터·같은 서버에서 상위 모델은 정상 표시). 서버가 반환한 조문 필드에는 영향이 없어 이번 릴리스에서 대응하지 않았습니다.

## [0.41.0] - 2026-08-07

**검색·후보 footer 발화 신호 보강(contract 0.31.0)** — v0.40.0 배포 후 라이브 eval에서 AI가 검색 결과·후보 목록만 나열하는 답변에 하단 표준 안내를 표시하지 않는 사례 2건(P1·P3)이 발견되어, 배포 전 진단 프로브로 원인을 확정하고 보강합니다. 실측 결과 서버는 `standard_footer`를 정상 부착하고 있었고(호스트가 값을 글자 단위로 재현·재현 호출 전건 부착 확인), 실패 지점은 **호스트가 답변을 조립하는 시점에 부착 지시가 발화하지 않는 것**이었습니다(판단형 문안 대조에서도 미표시 — 예외 조항 오분류 단독 가설 기각). `contract_version` **0.30.0 → 0.31.0**(§5.32), 패키지 **0.40.0 → 0.41.0**. **입력 스키마 무변 → 웹 커넥터 재연결 불요.** 규정·매뉴얼 데이터와 기존 검색 결과·순서·절단은 byte 불변입니다.

### Added

- **`standard_footer_note` 인접 지시**(search_provision·suggest_review_sources 한정): `standard_footer` 바로 앞 키로, 목록형·0건 답변을 포함해 footer 값을 답변 마지막 줄들로 1회 표시하라는 지시를 데이터와 함께 전달합니다(매뉴얼 도구의 `manual_meta.standard_footer_note` 기성 패턴의 확장 — 인접 지시형 필드의 호스트 소비는 기존 릴리스에서 반복 실증). 매뉴얼 인용 시 매뉴얼 footer 우선·중복 부착 금지 문면을 내장해 기존 정상 동작(매뉴얼 footer 단독 선택·검색+상세 1블록)을 보호합니다. `get_provision_detail`·매뉴얼 도구에는 `standard_footer_note`가 부착되지 않습니다(응답의 `contract_version` 값 갱신 외 기존 필드 무변).
- **footer-먼저 2단 폴백**: note+footer 포함 직렬화가 16,000자 이하면 둘 다, 초과하면 footer만으로 재시도해 **v0.40.0의 footer 부착 커버리지를 그대로 보존**하고, 그래도 초과하면 기존과 동일하게 원본 무변경 생략합니다(note 단독 부착 없음).

### Changed

- **하단 안내 예외 문면 정밀화**(서버 지시문): "규정 내용 판단이 없는 답변에는 붙이지 마십시오"가 목록·후보 나열형 답변에도 미부착 명분이 될 수 있던 것을, 예외를 서버 운영 안내(도구 상태·설치·연결)로 한정하고 "규정 검색 결과나 조문·별표·검토 후보를 안내하는 답변에는 목록형이거나 검색 결과가 없어도 standard_footer 선택·생략 폴백 규칙에 따라 붙이십시오"로 교체했습니다(footer가 응답에서 소실된 경우의 직접 표시 폴백까지 명시 지칭). `review_regulation` 템플릿·README 임베드에는 해당 예외 문장이 없어 무접촉입니다.

## [0.40.0] - 2026-08-06

**검색·후보 경로 표준 안내 확대(contract 0.30.0)** — 답변 하단 표준 안내(`standard_footer`)가 `search_provision`·`suggest_review_sources` 응답에도 동반됩니다. 지금까지는 규정 상세(`get_provision_detail`·v0.29.0)와 매뉴얼 도구에만 있어, AI가 검색·후보 목록만 보고 답하는 경우 하단 안내가 도달하지 않는 경로가 남아 있었습니다(v0.29.0에서 명시 이연·이번에 해소). `contract_version` **0.29.0 → 0.30.0**(§5.31), 패키지 **0.39.0 → 0.40.0**. **입력 스키마 무변 → 웹 커넥터 재연결 불요.** 규정·매뉴얼 데이터는 byte 불변입니다.

### Added

- **검색·후보 응답 standard_footer**: 두 도구의 정규 반환 전부(정상·0건·키워드 추출 실패 안내·일부 규정 오류 병존 포함 — 무키 인증 오류·invalid_query 진입부 오류 envelope 제외)에 기존과 동일한 2줄 안내(법령 확인 + KISTEP 매뉴얼 원문 안내)를 부착합니다. 기존 whole-or-omit 규약 그대로 응답이 크기 상한(16,000자)에 달한 극단 케이스에만 생략되며, **기존 검색 결과·순서·절단은 byte 단위로 보존**됩니다(예산 선반영 없음).

### Changed

- **하단 안내 선택 지시 확대**: 서버 지시문·`review_regulation` 템플릿의 footer 값 선택 규칙에서 매뉴얼 비인용 분기가 `get_provision_detail`만 지목하던 것을 규정 도구 3종(search·suggest·detail)으로 확대했습니다(세 도구의 값이 동일 문자열이라 기존 "아무 하나만" 규칙 유지).
- **생략 폴백 문면 교정**: footer가 생략된 응답에서 AI가 직접 표시할 폴백 문면에 KISTEP 매뉴얼 원문 안내 줄을 추가해 실제 footer 2줄과 일치시켰습니다(v0.30.0에서 footer가 2줄이 될 때 폴백이 미동기였던 갭).

### Internal

- 로드맵 정정: "시설장비 표준지침(admrul 2100000278230) 규정 트랙 등록(64→65)" 후보는 전제 오류로 소멸 — 해당 규칙은 v0.1.0부터 `facility_equipment_standard`로 기등록·LIVE 재프로브(2026-08-06) 전 필드 현행 일치. 부수 확인된 현행 고시 제100조↔표준지침 조번호(제6조/제7조) 불일치는 정부 규정 원문 간 사안으로 서버 무조치(3-AI 3/3 — verbatim 원칙·별권 4 붙임 라벨 기수록).

## [0.39.0] - 2026-08-06

**혁신법 매뉴얼 별권 4 「연구시설･장비비 통합관리제 운영･관리 매뉴얼」 수록(contract 0.29.0)** — 연구시설·장비비 통합관리제의 지정·계정 운영·적립·사용·점검·지정취소 실무 해설을 매뉴얼 도구로 조회할 수 있게 됩니다. 이로써 「국가연구개발혁신법 매뉴얼」 **본권·별권 1~4 전권이 수록**됩니다. 기존 매뉴얼 도구 2종에 통합되어 **새 도구가 추가되지 않습니다**. `contract_version` **0.28.0 → 0.29.0**(§5.30), 패키지 **0.38.0 → 0.39.0**. **입력 스키마 무변 → 웹 커넥터 재연결 불요.** 기존 매뉴얼 데이터 5종(본권·별권 3·2·1·과제평가 표준지침)은 byte 불변입니다.

### Added

- **별권 4 데이터 11단위**(`manual_b4.json`·39,297자·인쇄 3~50쪽 전체): 별권 4는 장(章) 없는 평면 편제라 **최초의 단일 레벨 id**를 사용합니다 — `b4-0`(개요)·`b4-1`~`b4-9`(본문 Ⅰ~Ⅸ — 용어의 정의·시행기관의 지정·적용 범위·통합관리계정의 개설·운영·계상·지급·적립·사용(집행)·관리·감독·지정 취소·기타 관리)·`b4-ref-1`(붙임 관련 규정 발췌). 참고 박스 3건(연구개발 목적기관·전산시스템 구축요건·ZEUS 정보 연계)은 본문 사이에 삽입된 원문 배치 그대로 부모 절에 병합되고 하위 제목으로 검색에 도달합니다.
- **독립 로더·장애 격리**: `manual_b4.json` 손상·누락 시 별권 4 조회만 `manual_b4_unavailable`로 격리되고 기존 매뉴얼 5종·규정 도구는 정상 동작합니다(소스 가용성 2^6=64조합 전수 테스트).
- **붙임 구판 발췌 사실 라벨**: 붙임의 「연구개발비 사용 기준」 발췌는 원문 부칙 표기 기준 고시 제2023-49호(2023.12.28.)까지 반영된 스냅샷으로, 현행 고시(제2026-38호·시행 2026-05-06)와 문구가 다른 부분이 확인되었습니다(제100조가 인용하는 표준지침 조번호, 제101조의 '참여연구자' 용어 — 2026-08-06 원문 대조). 응답 meta에 이 사실과 "조문 인용은 현행 원문 기준" 안내, 별지 제13~17호서식의 현행 확인 경로(law.go.kr)를 기재합니다.

### Changed

- **매뉴얼 장애 안내 문면 정비(5곳)**: 매뉴얼 데이터 로드 실패 안내의 "규정 도구는 정상입니다"류 문구는 그 시점에 규정 도구를 검사하지 않은 단정이라, 검증된 사실만 말하는 격리 문면("이 오류는 해당 매뉴얼 데이터 로드에 한정되며 규정 도구 경로에는 전파되지 않습니다")으로 교체했습니다(직전 릴리스 검토에서 발굴된 개선 항목).

### Fixed

- **본권 데이터 구조 손상 시 오류 격리 보강**: 본권 `manual_body.json`이 유효한 JSON이면서 내부 구조가 손상된 경우(예: 절 원소가 null) 종전에는 예외가 도구 호출로 새어 나갔습니다 — 별권 로더들과 동형의 try/except로 `manual_unavailable` 안내 응답으로 격리했습니다(구현 검토에서 적발된 기존 결함·검증 규칙 추가 없음·정상 데이터 거동 불변).

### Internal

- 추출 파이프라인 `scripts/extract_manual_b4.py`(승인 PDF sha256 고정·편제 상수 동결·쪽별 헤더/마커 정확 2줄 제거·전체 장부 검증[원본 = 수록 + 제거 + 제외 일치]·참고 표제 실재 검증·실질 표 페이지 자동 기록·fail-closed·byte-identical 재현 검증). 붙임 발췌 vs 현행 고시 제100~111조 정규화 대조 수행(구조 표면 전건 정합·문구 차이 2건 사실 기재). 검색 희석 실측(기존 대표 질의 반환 잠식 0·접두 보존 잠금). b4-ref-1의 검색 oversized 플래그(본문 기준 false)와 상세 청크(응답 전체 기준 청크 2)의 판정 기준 차이는 기존 절과 동형의 문서화된 거동(§5.30). 구현 diff 적대검토(2-AI) 반영: 등록 prompt description·README 도구 표·수록 범위·API 계약 §4 오류 표(manual_eval_unavailable 소급 등재 포함)의 별권 4 정합화 + 등록 prompt description 잠금 테스트 신설. pytest 710 → 772.

## [0.38.0] - 2026-08-06

**「국가연구개발 과제평가 표준지침」(25.12) 수록(contract 0.28.0)** — 과제평가(선정·단계·특별·최종)의 범부처 표준 기준·절차·Q&A를 매뉴얼 도구로 조회할 수 있게 됩니다. 이 지침은 연구성과평가법 제13조에 따라 과학기술정보통신부가 마련하는 범부처 표준지침으로, 법령·행정규칙 DB(국가법령정보센터 OpenAPI)에 등재되어 있지 않아(2026-08-06 확인) 본 서버의 유일한 제공 경로가 매뉴얼 트랙입니다. 매뉴얼 트랙 최초의 비(非)혁신법-매뉴얼 독립 자료이며 기존 매뉴얼 도구 2종에 통합되어 **새 도구가 추가되지 않습니다**. `contract_version` **0.27.0 → 0.28.0**(§5.29), 패키지 **0.37.0 → 0.38.0**. **입력 스키마 무변 → 웹 커넥터 재연결 불요.** 기존 매뉴얼 데이터 4종(본권·별권 3·2·1)은 byte 불변입니다.

### Added

- **과제평가 표준지침 데이터 15단위**(`manual_eval.json`·54,507자·인쇄 1~60쪽 전체): id `eval-<장>-<절>`(제1장 5절·제2장 단일 유닛·제3장 3단위) + `eval-ref-1~6`(참고 — 전문기관 현황·성과목표 설정·연구개발 단계·평가 주안점·경쟁형 R&D·과제평가 Q&A). 선정·단계·특별·최종평가는 제3장 2. 평가단계별 공통추진사항(`eval-3-3`·대형 절이라 청크 분할)의 하위 구획으로 검색 도달합니다.
- **독립 로더·장애 격리**: `manual_eval.json` 손상·누락 시 이 자료 조회만 `manual_eval_unavailable`로 격리되고 기존 매뉴얼 4종·규정 도구는 정상 동작합니다(소스 가용성 2^5=32조합 전수 테스트).
- **문서 성격 사실 라벨**: 응답 meta에 발행 주체(과기정통부·KISTEP)·법적 근거(연구성과평가법 제13조·시행령 제17조)·판권지 "개정(안)" 표기 사실·"각 부처는 이 표준지침을 고려해 자체 평가지침을 마련하므로 구체 과제에는 소관 부처·전문기관의 평가지침·공고·평가계획이 함께 적용됨" 안내를 기재합니다(현행성·확정성 단정 없음).
- **footer per-source 문면**: 답변 하단 표준 안내의 3번째 줄이 자료별 문면을 지원합니다 — 이 지침 인용 응답은 "※ 지침 해설 부분은 「국가연구개발 과제평가 표준지침」을 참고한 설명입니다…"로 표기되어 기존 고정 문면(혁신법 매뉴얼 명시)의 출처 오귀속을 차단합니다. 기존 4소스 응답 문면은 byte 불변입니다.
...
**resolver 안전성 정비 — 미시행 규정 오제공 차단 + 개정 예정·확인 실패 고지(contract 0.27.0)** — 최신 판본 확인(search-first resolve)이 시행일이 아직 오지 않은 판본을 현행처럼 선택하던 결함을 바로잡습니다. 2026-08-06 라이브 실측에서 「국가연구개발정보처리기준」 조회가 미시행 판본(고시 제2026-47호·시행 2026-08-20)의 본문을 "개정 반영"으로 표기하며 반환하고 있었습니다. 이제 시행일이 지난 판본만 현행으로 선택되며, 예정 판본이 공포되어 있으면 그 사실이 응답에 명시됩니다. `contract_version` **0.26.0 → 0.27.0**(§5.28), 패키지 **0.36.0 → 0.37.0**. **입력 스키마 무변 → 웹 커넥터 재연결 불요.** 규정 manifest·매뉴얼 데이터 byte 불변.

### Fixed

- **미래 시행 행 필터**: 최신 판본 확인 시 제목·소관부처가 일치하는 검색 행 중 **시행일자가 오늘(KST) 이하인 행만** 현행 후보로 선택합니다. 종전에는 시행일 최대 행을 무조건 선택하여, 개정 예고(공포·미시행) 판본이 국가법령정보 검색에 노출되는 즉시 미시행 본문이 현행처럼 제공될 수 있었습니다(64개 규정 공유 경로의 구조적 결함). 일치 행이 전부 미래이면 등록 스냅샷(구 판본 = 현행)을 기준으로 응답합니다. 시행일자 형식이 비정상인 행은 미래로 단정하지 않아 종전 가용성이 유지됩니다.

### Added

- **`upcoming_revision`(조건부)**: 시행 예정 판본이 공포되어 있으면 상세·검색 응답에 완성형 안내 1문장을 포함합니다 — 예정 시행일·새 판본 문서 ID·"본 응답은 현행 시행본 기준" 명시. 예정 판본이 여럿이면 가장 이른 것만 안내합니다.
- **`resolve_fallback_notice`(조건부)**: 최신 판본 확인이 실패(국가법령정보 검색 실패 또는 일치 문서 없음)하여 등록 스냅샷 기준으로 응답할 때, 그 사실과 스냅샷 식별자(문서 ID·시행일)를 상세 응답에 명시합니다(종전에는 fallback 발동이 표시되지 않아 등록 시행일이 정상 확인 값처럼 보였습니다).

### Internal

- 사후주입 헤드룸 `_ANNEX_DETAIL_HEADROOM`·`MANUAL_DETAIL_HEADROOM` 300 → 600(신규 고지 2종 주입 대비·parity 잠금 유지 — 예산 16,000자 초과 없음 보장·강등 경계 보수화는 안전 방향). resolver 필터·고지 잠금 테스트 12건 추가. 「국가연구개발정보처리기준」 manifest는 의도적 무변(신 고시 반영은 시행일 2026-08-20 도래 후 별도 릴리스 — 본 정비로 그때까지 구 판본이 현행으로 정확히 제공되고 예정 사실이 함께 안내됩니다). pytest 643 → 655.
...
- annotations 잠금 테스트 15건(registry·wire 직렬화 양면·7도구 전수). 국가연구개발정보처리기준의 신 고시(2100000283100)는 시행일(2026-08-20) 도래 후 별도 갱신 예정 — 미시행 판본을 fallback으로 고정하지 않기 위한 의도적 보류(평시 검색 경로는 종전과 동일). pytest 626 → 643.
...
참고: 2026-08-03 로드맵의 R4 최초 예약 버전은 v0.34.0이었으나, 2026-08-05 structure_notice 릴리스(v0.34.0)가 그 번호를 사용하여 본 릴리스는 v0.35.0으로 순연됐습니다(범위·의도 불변).

### Added

- **별권 1 데이터 26단위**(`manual_b1.json`·76,353자): id `b1-<장>-<절>`(일반 절 22) + `b1-5-1`(제5장 FAQ — 절 편제가 없는 장의 단일 유닛·Q1~Q9 문항 줄을 검색 제목군으로 등재) + `b1-ref-1~3`(참고). 장 표제 간지 5쪽과 빈 쪽 7쪽은 미수록이며(응답 meta의 excluded_note에 기재) 본문 마커 79쪽 전체를 수록합니다.
- **검색·조회 통합**: `search_manual`이 4개 자료(본권·별권 3·별권 2·별권 1)를 병합 검색하고(`source` 값 "b1" 추가·scanned_sections 107), `get_manual_section`이 `b1-…` id를 라우팅합니다. 별권 1 데이터 손상 시 신규 오류 코드 `manual_b1_unavailable`로 별권 1 조회만 격리됩니다(본권·별권 2·3·규정 도구 무영향).
- **규범성 안내(별권 1 전용)**: 원문이 인용하는 고시 번호(제2026-5호)가 발간 시점 표기임을 사실 기재하고, 학생인건비 이자 처리 서술이 구판 고시 제75조 구조 기준이라는 안내(현행은 제75조제2항제1호로 이동)를 `law_priority_extra`로 동반합니다. 계상기준·지급액 등 구체값은 연구개발비 사용 기준 원문 교차 확인을 안내합니다.
- **표·산식 구조 안내 4절**: 지급 개념도·지급체계 표(막대 그림)·지정 현황 다열 표(관리유형 열 귀속)·점검 체크리스트(가/부 판정란)와 전산시스템 구축 예시 화면(이미지 전용)의 구조 손실을 `table_structure_notes`로 사실 기재 — v0.34.0 `structure_notice` 완성형 블록이 데이터만으로 부착됩니다(코드 무변). 핵심 구체값(계상기준 월 130만/220만/300만 원·제91조의2 잔액 산식)은 텍스트로 보존·결속 정상을 렌더 대조로 확인했습니다.
- **대형 참고 청크**: 참고3 점검 자료집(25,384자)은 포인터 + 청크 3개(무손실 재조립·각 청크에 구조 안내 동반)로 제공됩니다.
...
**v0.33.0 배포 후 확인된 응답 안전성 결함 2건 최소 보강(contract 0.25.0)** — ①표·산식 구조 손실 안내가 AI 답변에 실제로 전달되도록 완성형 블록으로 승격하고 ②오류 응답이 사용자 입력을 그대로 되돌려 줄 때 API 키가 노출될 수 있는 경로를 마스킹합니다. `contract_version` **0.24.0 → 0.25.0**(§5.26), 패키지 **0.33.0 → 0.34.0**. **입력 스키마 무변 → 웹 커넥터 재연결 불요.** 매뉴얼 데이터 3종(본권·별권 3·별권 2)은 byte 불변입니다.

### Added

- **`structure_notice` 완성형 안내 블록**(`get_manual_section`): 표·산식 구조가 추출 텍스트에 보존되지 않은 절(별권 3 제4장 2절, 별권 2 제2장 1절·제3장 2절·부록)의 본문 응답에, 답변에 그대로 옮길 수 있는 완성형 안내 블록과 표시 지시(`structure_notice_note`)를 추가했습니다. 종전에는 같은 내용이 `warnings` 배열로만 제공되어 배포 후 검증에서 AI 답변에 반영되지 않는 사례가 관측됐습니다(같은 응답의 citation·하단 표준 안내 같은 완성형 블록은 반복적으로 복사되는 것이 관측되어 온 형태입니다 — 실제 개선 효과는 배포 후 검증으로 확인합니다). 청크 응답에는 "절 전체 기준" 주의 줄이 함께 붙습니다.

### Fixed

- **오류 응답 self-echo 마스킹**: `invalid_provision_id`·`invalid_section_id`·규정 상세의 manifest 불일치 `not_found` 등 오류 문면이 사용자 입력을 그대로 되돌려 주는 경로 5곳에 키 마스킹을 적용했습니다. 사용자가 실수로 자신의 API 인증 키를 도구 인자에 붙여넣은 경우에도 오류 메시지에 키가 재출력되지 않습니다(본인 세션 한정 echo였으며 교차 사용자 노출·실제 유출 사례는 없습니다). 키가 포함되지 않은 일반 오류 문면은 종전과 동일합니다.
- README의 오래된 테스트 수 안내(197)를 현행(575)으로 정정했습니다.

### Internal

- `structure_notice` 부착은 크기 판정 이후 마지막 단계로 수행되며(기존 절의 응답 형태 불변), 부착 결과가 응답 예산(16,000자)을 초과하면 원본 무변경으로 생략됩니다(이 경우에도 warnings에 같은 내용이 남습니다). 실측 최대 케이스(별권 3 제4장 2절 청크 1 = 15,799자)를 테스트로 잠갔습니다. 기존 `warnings` 표면은 그대로 유지됩니다. pytest 550 → 575.

## [0.33.0] - 2026-08-04

**혁신법 매뉴얼 별권 2 「국가연구개발사업 기술료 제도 매뉴얼」 수록(contract 0.24.0)** — 기술료 제도(징수·정부납부기술료·감면·사용)와 협약 시 기술기여도 산정·검증 가이드라인 해설을 매뉴얼 도구로 조회할 수 있게 됐습니다. 종전에는 시행령 제38조~제41조·기술료 통합요령 등 조문 원문만 제공되어 제도 해설·절차 층이 없었습니다. 별권 2는 **26.7 게시 세트·3개 장 14개 절 + 부록 1건·인쇄 1~12쪽**이며, 기존 매뉴얼 도구 2종에 통합되어 **새 도구가 추가되지 않습니다**. `contract_version` **0.23.0 → 0.24.0**(§5.25), 패키지 **0.32.0 → 0.33.0**. **입력 스키마 무변 → 웹 커넥터 재연결 불요.**

### Added

- **별권 2 데이터 수록**(`manual_b2.json` 신설·15단위·9,531자): 제1장 제도 개요(정의·목적·근거) · 제2장 제도 운영(운영체제·기술료의 징수·정부납부기술료·감면·사용) · 제3장 정부납부기술료 세부 기준(납부대상·납부 기준·납부 기간·감면·납부 수단·관련 서식) · [부록] 연구개발과제협약 시 기술기여도 산정 및 검증 가이드라인. 본권·별권 3 데이터는 그대로 유지됩니다.
- **통합 검색 확장**: `search_manual`이 본권·별권 3·별권 2를 함께 검색합니다. 각 매치의 `source`에 "b2"(별권 2)가 추가되고 소스별 매치 수도 함께 제공됩니다. 정렬은 기존 규칙(제목 매치 우선·같은 단계에서 본권 우선)을 유지하며 **별권 2에 매치가 없는 검색어의 결과는 종전과 동일합니다**(v0.32.0 기준선과 응답 전체 대조로 검증).
- **절 본문 조회**: `get_manual_section`에 별권 2 id 형식(`b2-3-2`·`b2-ref-1`)을 추가했습니다. 입력 파라미터 구조는 그대로입니다(문자열 값만 확장).
- **규범성 안내 확장**(별권 2 응답 한정): ①기술료율·납부 상한·납부 기간 등 구체값은 시행령 제38조~제41조 원문으로 교차 확인 ②[부록]의 기술기여도 산정 예시·협약 단계 가이드라인은 개별 사안에 그대로 일반화 불가 ③**이 매뉴얼의 '기술료등납부의무기관'은 현행 시행령에서 '정부납부기술료납부의무기관'으로 변경**되었으므로 현행 용어·적용 범위는 시행령 제38조·제39조 원문 기준으로 확인(현행 조문 실측 검증 완료).
- **표 구조 고지**: 제2장 1절([그림 1] 운영체제 도식이 이미지 전용), 제3장 2절(기준표 '분류' 열의 세로 병합 셀), [부록](기술기여도 분수식의 분자·분모 관계·인쇄 12쪽 표의 빈 셀 열 귀속)에서 PDF 추출 텍스트로는 구조 정보가 확인되지 않는다는 사실을 응답에 명시합니다.

### Changed

- 별권 2의 판번(26.7)은 KISTEP 게시 세트·파일명 기준이며, **별권 2 원문에도 법령 기준일이 명시되어 있지 않다**는 사실을 응답 안내에 그대로 표기합니다(본권 기준일 2026.6월을 별권에 준용하지 않습니다). 본권·별권 3 응답 문면은 종전과 글자 단위로 동일합니다.
- 세 자료가 한 검색 결과에 함께 나오면 하단 표준 안내의 판번 줄이 세 자료를 병기한 완성형으로 제공되고, 자료별 판번·기준일이 `manual_meta.sources`로 구분 제공됩니다.
- 도구 설명·서버 안내·검토 템플릿·README의 수록 범위 표기를 갱신했습니다(매뉴얼 별권 미커버 3종 → 2종: 학생인건비·연구시설장비).

### Internal

- 별권 2 전용 추출 파이프라인 `scripts/extract_manual_b2.py` 신설(본권·별권 3 스크립트 무변): 승인 PDF sha·물리 쪽수·쪽별 추출 텍스트 sha 12건·총 자수·목차/본문 제목 이중 스냅샷(승인 불일치 1건 한정)·부록 소제목 줄 단위 검증·산출물 세트 원자성. 결정론 2회 byte-identical·12쪽 전수 무손실 대조·결속 객체(표·분수식·도식) 렌더 대조·변조 주입 5종 전건 BLOCK 확인.
- **교차 소스 층 일반화**: 별권 소스 descriptor 목록으로 검색 병합·가용성 8조합 envelope·부분 장애 경고·상세 라우팅·혼합 meta 조립을 처리합니다(로더·캐시는 자료별 독립 유지 — 한 자료의 결함이 다른 자료 서빙에 전파되지 않습니다). 신규 오류 코드 `manual_b2_unavailable`.
- 별권 2 로더에 강화 검증 추가(id 유일·프리픽스·인덱스 연속·항목 수 일치). acceptance spec `v0_33_0.py` 신설. v0.32.0 기준선 golden 픽스처로 보존 표면 실대조. pytest 511 → 550.

## [0.32.0] - 2026-08-03

**혁신법 매뉴얼 별권 3 「국가연구개발사업 제재처분 가이드라인」 수록(contract 0.23.0)** — 제재처분(참여제한·제재부가금·환수)의 절차·기준·재검토 쟁점 해설을 매뉴얼 도구로 조회할 수 있게 됐습니다. 종전에는 시행령 별표 6·7 조문 원문만 제공되어 절차와 판단 기준 해설이 없었습니다. 별권 3은 **26.7 게시 세트·5개 장 22개 절 + 부록 1건·인쇄 1~89쪽**이며, 기존 매뉴얼 도구 2종에 통합되어 **새 도구가 추가되지 않습니다**. `contract_version` **0.22.0 → 0.23.0**(§5.24), 패키지 **0.31.0 → 0.32.0**. **입력 스키마 무변 → 웹 커넥터 재연결 불요.**

### Added

- **별권 3 데이터 수록**(`manual_b3.json` 신설·23단위·87,844자): 제1장 소개 · 제2장 제도 개요(법적 근거·주요용어·혁신법 변경사항) · 제3장 절차(조사·검증부터 국세 강제징수까지 9개 절) · 제4장 기준(일반 기준·사유별 가중·감경 세부기준) · 제5장 재검토 주요쟁점 5건 · 부록(신고·보상 제도 안내). 본권 데이터는 그대로 유지됩니다.
- **통합 검색**: `search_manual`이 본권과 별권 3을 함께 검색합니다. 각 매치에 `source`("main"=본권 / "b3"=별권 3)와 자료별 완성형 인용구가 붙고, 소스별 매치 수(`total_matched_by_source`·`returned_by_source`·`searched_sources`)가 함께 제공됩니다. 정렬은 기존 규칙(제목 매치 우선)을 유지하며 같은 단계에서는 본권이 앞섭니다 — **별권 3에 매치가 없는 검색어의 결과는 종전과 동일합니다.**
- **절 본문 조회**: `get_manual_section`에 별권 3 id 형식(`b3-4-2`·`b3-ref-1`)을 추가했습니다. 입력 파라미터 구조는 그대로입니다(문자열 값만 확장).
- **규범성 안내 확장**(별권 3 응답 한정): 참여제한 기간·제재부가금 비율 등 구체값은 시행령 별표 6·별표 7 원문으로 교차 확인하라는 안내와, 제5장 쟁점·검토결과는 개별 사안별 결과라 유사 사안에 그대로 일반화할 수 없다는 안내가 포함됩니다.
- **표 구조 고지**: 제4장 2절(사유별 가중·감경 세부기준) 조회 시, 세로 병합 셀의 적용 범위와 감경/가중요소 예시표의 경계가 PDF 추출 텍스트에서 구분되지 않는다는 사실을 응답에 명시합니다 — 해당 부분은 표기된 인쇄쪽 원문 또는 시행령 별표 원문으로 확인해야 합니다.
- **부분 장애 격리**: 별권 3 데이터만 문제가 있으면 본권 검색은 정상 동작하고 `source_warnings`로 부분 결과임을 알립니다(별권 3 절 조회만 `manual_b3_unavailable`). 반대의 경우도 같습니다.

### Changed

- 별권 3의 판번(26.7)은 KISTEP 게시 세트·파일명 기준이며 PDF 본문에 판번 표기가 없다는 사실, 그리고 **별권 3 원문에는 법령 기준일이 명시되어 있지 않다**는 사실을 응답 안내에 그대로 표기합니다(본권 기준일 2026.6월을 별권에 준용하지 않습니다). 본권 응답 문면은 종전과 글자 단위로 동일합니다.
- 본권·별권이 한 검색 결과에 함께 나오면 하단 표준 안내의 판번 줄이 두 자료를 병기한 완성형으로 제공되고, 자료별 판번·기준일이 `manual_meta.sources`로 구분 제공됩니다.
- 도구 설명·서버 안내·검토 템플릿·README의 수록 범위 표기를 갱신했습니다(매뉴얼 별권 미커버 4종 → 3종).

### Internal

- 별권 전용 추출 파이프라인 `scripts/extract_manual_b3.py` 신설(본권 스크립트 무변): 목차 스냅샷·검색 제목군 46개 전수 대조·같은 쪽 다중 절 분할·전 검증 fail-closed BLOCK·원자적 쓰기. 결정론 2회 byte-identical·89쪽 전수 무손실 대조(토큰 소실 0)·표 밀집 8쪽 시각 대조(오결속 0)·대표 질의 18종 랭킹 계측.
- 별권 데이터 손상(구조 이상) 시 예외 전파 없이 격리되도록 로더 방어. 오류·경고 문면은 실제 확인된 소스 상태만 진술합니다.
- acceptance spec `v0_32_0.py` 신설. pytest 481 → 511.
