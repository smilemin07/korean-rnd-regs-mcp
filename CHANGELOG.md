...
- ★이 설정이 줄이는 것은 **커널 네트워크 인터페이스 노출**(LAN 직접 접근)입니다. 같은 호스트에서 loopback으로 대신 접속해 주는 프로세스를 통한 도달은 바인딩 주소로 막을 수 없습니다 — 실측(2026-08-22): Synology Tailscale 패키지는 `--tun=userspace`로 동작해 tailnet 요청을 `127.0.0.1`로 프록시하므로, `FASTMCP_HOST=127.0.0.1` 후에도 tailnet 구성원(운영자 인증 기기)은 포트에 닿습니다. 이 경로는 본 릴리스 범위 밖이며 Tailscale ACL 등 운영 정책의 후속 과제로 기록합니다. 배포 게이트도 이에 맞춰 "Tailscale 직접 호출 거부"(잘못된 대리 지표)가 아니라 "LAN IP 직접 호출 거부 + loopback 단독 LISTEN + 터널 정상 + 무키 406"으로 검증했습니다.
