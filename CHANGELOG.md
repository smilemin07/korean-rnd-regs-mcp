...
**혁신법 매뉴얼 본권 26.7판 현행화(contract 0.22.0)** — 임베드 매뉴얼 데이터를 KISTEP 2026년 7월 개정판으로 전체 교체하고 26.4판 데이터를 삭제했습니다. 26.4판은 법령 시행일 2026.3월 기준이라 시행령(2026-05-06 개정)·시행규칙·연구개발비 사용기준의 최근 개정이 반영되지 않은 상태였습니다. 26.7판은 **법령 시행일 2026.6월 기준·43개 절(신설 포함)·인쇄 1~332쪽**입니다. `contract_version` **0.21.0 → 0.22.0**(§5.23), 패키지 **0.30.0 → 0.31.0**. **입력 스키마 무변 → 웹 커넥터 재연결 불요.**

### Changed

- **매뉴얼 데이터 26.4판 → 26.7판**(manual_body.json 전체 교체): 41절(39절+참고 2건)·380,267자 → **43절(40절+참고 3건)·388,172자**. 신설: 제3장 제12절 「연구혁신비 사용용도 및 사용기준」·[참고] 3 「국가연구개발사업 행정서식 관리체계」. 26.7판부터 [부록] 관련 서식은 별도 PDF로 분리 발간되어 본권에 없습니다. 판번·기준일·인용·하단 안내의 판 표기는 데이터 기반이라 함께 갱신됩니다.
- **★제3장 절 번호 변경(재번호 8건)**: 제12절 신설로 26.4판의 제12~19절(간접비~연구시설·장비비 통합관리)이 26.7판에서는 제13~20절이 됐습니다. 절 id(section_id)는 현행 수록 판의 장-절 번호이므로 함께 이동합니다(예: 간접비 = 구판 "3-12" → 현행 "3-13").
- search_manual 도구 설명(43개 절·인쇄 1~332쪽)·get_manual_section 미존재 안내의 유효 범위(제3장 3-1~3-20·참고 ref-1~ref-3)·README 도구 설명(26.7판·2026.6월 기준) 갱신.

### Added

- **`manual_meta.renumbering_note`**(get_manual_section 비오류 응답·재번호 영향 절 3-12~3-20 한정): 위 재번호 사실과 "구판 번호로 조회했다면 section_title·citation을 확인하고 search_manual로 다시 찾으라"는 안내 — 구판 번호를 기억한 호출이 오류 없이 다른 주제를 받는 전환기 혼동을 줄입니다. 값·대상 절 목록은 데이터 파일에 있어 다음 판에서 재번호가 없으면 데이터만 비워 사라집니다. 검색·오류 응답에는 붙지 않습니다.

### Internal

- 재추출 파이프라인 26.7판 대응: EXPECTED 목차 스냅샷·판 메타(기준 법령 4건 실측)·본문 끝 332쪽·부록 없음(None) 갱신, 목차 점선 1개 엔트리 대응(`·{2,}`→`·{1,}` — 26.7 최장 제목 절 실측), "본 권" 표제 줄 스킵, 절 수 assert 동적화, 검수용 임시 경로 추출 지원. 결정론(2회 byte-identical)·무손실(표 밀집 6개 절 토큰 대조 누락 0)·경계 감사 전건 확인.
- acceptance spec `v0_31_0.py` 신설. pytest 474 → 481.

## [0.30.0] - 2026-08-02

**출처·원문 확인 경로 정비(contract 0.21.0)** — 답변 하단 표준 안내를 두 갈래로 정비했습니다. ① 배포 후 관측(6회)에서 법률만 다룬 답변이 하단 안내의 "행정규칙" 단어를 2/2 삭제하는 현상이 확인되어, 첫 줄을 어떤 답변 맥락과도 충돌하지 않는 **중립 문면("관련 규정 원문")**으로 교체했습니다. ② 서버가 임베드·재배포하는 「국가연구개발혁신법 매뉴얼」의 **원문 배포처(KISTEP) 안내**를 신설했습니다 — 하단 안내 공통 2번째 줄(홈페이지 안내형·URL/판번 미포함) + 매뉴얼 응답 메타의 게시물 URL(기계 가독 출처). 서버 미수록 **별권 4종·서식**을 사용자가 원문에서 받을 경로가 생깁니다. `contract_version` **0.20.0 → 0.21.0**(§5.22), 패키지 **0.29.0 → 0.30.0**. **입력 스키마 무변 → 웹 커넥터 재연결 불요.**

### Changed

- **하단 표준 안내 첫 줄 중립화**: "※ 정확한 최종 확인은 국가법령정보센터(law.go.kr)의 **관련 규정** 원문을 기준으로 해주시기 바랍니다." (구: "법령·행정규칙 원문") — 프롬프트 폴백 리터럴·README 임베드 동기 교체.
- **하단 표준 안내 2번째 줄 신설(전 경로 공통)**: "※ 「국가연구개발혁신법 매뉴얼」 등 연구행정 관련 매뉴얼 원문은 KISTEP 홈페이지(www.kistep.re.kr)에서 확인하시기 바랍니다." — 규정 상세 답변 2줄 / 매뉴얼 인용 답변 4줄이 되며, 처음 두 줄이 동일 문자열이라 중복 부착 방지 구조는 그대로 유지됩니다.

### Added

- **`manual_meta.source_url`**(매뉴얼 도구 2종 비오류 응답): 임베드한 판(26.4)의 KISTEP 게시물 전체 URL — 출처 귀속·"정확히 어디서 받나" 질문에 대한 날조 방지 근거. 값은 데이터 파일(manual_body.json) meta 한 곳에서 판번과 함께 관리됩니다.
- **README — ChatGPT 웹 안내(Option 4)**: 개발자 모드 등록 절차(2026-08 실계정 검증 완료)와 미검증 MCP 연결 가능성 주의 문구, Troubleshooting Q6(승인 창 경고·"쓰기" 분류 고지).

### Internal

- 재추출 스크립트(extract_manual.py)에 `--source-url` 필수 인자(https+kistep.re.kr 검증) — 판 갱신 시 본문과 출처 URL이 항상 함께 생성. 갱신 절차를 일정 기반("연 1회")에서 **새 판 게시 기반**으로 정정(2026년 4월·7월 연 2회 개정 실증).
- acceptance spec `v0_30_0.py` 신설. pytest 474.

## [0.29.0] - 2026-07-25

**규정 상세 응답 하단 표준 안내 — A-1 경로 응답 구조화(contract 0.20.0)** — v0.28.0 배포 후 eval에서 같은 세션 A/B 대조가 확인됐습니다: 서버가 완성형 안내를 준 매뉴얼 경로는 하단 안내 부착 2/2, AI가 지시만으로 직접 조립해야 하는 규정 답변 경로는 0/2(직전 릴리스 포함 누적 0/5). 이번 릴리스는 그 마지막 갭을 메웁니다 — **매뉴얼을 쓰지 않은 규정 답변에도 서버가 완성형 안내를 공급**합니다. 성공 반환점 3곳에 헬퍼 1개를 거는 최소 변경이며, 기존 필드·거동은 완전 불변입니다. `contract_version` **0.19.0 → 0.20.0**(§5.21), 패키지 **0.28.0 → 0.29.0**. **입력 스키마 무변 → 웹 커넥터 재연결 불요.**

### Added

- **`standard_footer`**(get_provision_detail 성공 응답): "※ 정확한 최종 확인은 국가법령정보센터(law.go.kr)의 법령·행정규칙 원문을 기준으로 해주시기 바랍니다." 1줄 — 매뉴얼 3줄 안내의 첫 줄과 **동일 문자열**(단일 출처)이라 두 값이 겹칠 일이 없습니다. 문서레벨·조문·별표(포인터·청크·locate 포함) 비오류 응답 전부에 동반되고, 오류 응답에는 붙지 않습니다. 응답이 16,000자를 넘게 되는 극단 케이스에는 기존 응답을 전혀 건드리지 않고 생략합니다(whole-or-omit).

### Changed

- **하단 안내 프롬프트(2표면+README)**: "매뉴얼 인용 여부" 단일 분기로 단순화 — 인용했으면 매뉴얼 응답의 3줄 값(첫 줄에 법령 확인 포함 — 1줄을 덧붙이지 않음), 아니면 규정 응답의 1줄 값(여러 응답에 같은 값이면 아무 하나만), footer 블록은 답변당 정확히 1개. 구버전 응답 대비 리터럴 폴백은 보존.
- **테스트 466 → 472**: 헬퍼 경계(정확히 16,000 부착·16,001 무변경 생략)·성공 3경로 부착·오류 미부착·문면 단일 출처·프롬프트 단일 분기 문면 잠금.

### Coverage note

- 이 안내가 보장되는 범위는 **get_provision_detail 응답을 받은 답변**입니다. 검색·추천 결과만 보고 작성된 답변의 미부착 가능성은 남아 있으며(정상 흐름에서는 드묾), 후속 관측 대상으로 기록했습니다.

## [0.28.0] - 2026-07-25

**매뉴얼 인용 앵커·하단 표준 안내 응답 구조화(contract 0.19.0)** — v0.27.0 배포 후 eval과 매뉴얼-온리 유효성 테스트에서 근거 정확도(grounding)·도구 라우팅은 3/3이었으나, **인쇄쪽 인용 0/3·답변 하단 표준 안내 부착 0/3**(실질 규범성 고지는 본문에서 3/3 수행)이 반복 관측되었습니다. 형식 준수를 프롬프트 지시로 끌어올리는 방식은 수확체감이 확인되어(별표 방식 라벨 3라운드 1/3), 이번 릴리스는 **서버가 완성형 문자열을 만들어 제공하고 AI는 그대로 옮겨 적기만 하도록** 응답 구조를 바꿉니다. 기존 데이터(인쇄쪽·청크 페이지·판번)를 서버에서 재조립하는 것이라 **추가 조회·신규 파싱이 없고**, 기존 5종 규정 도구·검색·캐시·전송 계층은 완전 무접촉입니다. `contract_version` **0.18.0 → 0.19.0**(§5.20), 패키지 **0.27.0 → 0.28.0**. **입력 스키마 무변 → 웹 커넥터 재연결 불요.**

### Added

- **`citation`**(매뉴얼 도구 2종 응답): 「국가연구개발혁신법 매뉴얼(본권)」(26.4판) 제3장 제4절 …, 인쇄 p.204~218 형태의 **완성형 인용 한 줄**. 검색 결과는 매치 항목마다, 절 조회는 절 범위로, **청크 조회는 그 청크가 실제 담은 인쇄쪽 범위**로 표기됩니다(절 전체를 확인한 것처럼 범위를 넓혀 말하지 않도록).
- **`manual_meta.standard_footer`**: 답변 하단에 그대로 붙일 **완성형 안내 블록**. 응답이 매뉴얼 해설을 실제로 전달했으면 3줄(법령 원문 기준 확인 + 매뉴얼 해설 고지 + 인용 매뉴얼 판번·기준일), 전달하지 않았으면(검색 0건·본문 미수록 포인터) **법령 확인 1줄만** — 매뉴얼을 쓰지 않은 답변에 매뉴얼 인용 고지가 붙는 허위 출처 표시를 서버가 차단합니다. 사용 조건은 `standard_footer_note`로 함께 안내.

### Changed

- **프롬프트 3표면**(server instructions·`review_regulation` 템플릿 하단 안내·인용 지시): AI가 문구를 조립하던 지시를 "`standard_footer`·`citation` 값을 그대로 표시(직접 조립 금지)"로 치환하고, 롤백 등으로 해당 값이 없는 경우를 위한 기존 문구를 폴백으로 병기(README 임베드 동기화).
- **README**: Troubleshooting Q0에 커넥터 **⋮ → "Refresh tools list"**(연결 유지 갱신·권장) 경로 추가, 기능 표의 매뉴얼 도구 2종 설명 갱신.
- **테스트 448 → 466**: 여러 매뉴얼 응답 간 footer 선택 규칙, 결측·비정상 타입 citation 방어, 포인터·청크 전수 예산, citation 형식·참고 자료(ref-N) 장 생략·청크 페이지 앵커·결측 degrade, footer 3줄/1줄 분기, 오류 응답 미부착, size-tier 무회귀(전문 29·포인터 12), 예산 내 직렬화, 프롬프트 3표면 문면 잠금.

### Unchanged

- 기존 `notice`를 포함한 모든 응답 필드·5종 규정 도구·64개 지원 규정·검색/랭킹/fallback·fan-out·캐시·transport·외부 접속 URL.

## [0.27.0] - 2026-07-25

**R1 혁신법 매뉴얼 트랙 — 「국가연구개발혁신법 매뉴얼」(본권) 해설 도구 2종 신설(contract 0.18.0)** — 2026-06-14 "PDF 트랙 장기 보류" 정책의 조건부 해제(2026-07-24 Andy 결정·실현가능성 /disc 3-AI 조건부 GO·저작권 검토 완료) 첫 릴리스. 매뉴얼(26.4판·법령 시행일 2026.3월 기준) 본문 해설부(5장 39절+참고 2건·인쇄 1~326쪽·380,267자)를 절 단위 구조화 데이터로 패키지에 동봉하고, **신규 도구 2종 `search_manual`·`get_manual_section`**으로 검색·조회 제공. ★매뉴얼은 해설 자료이며 법령·행정규칙이 아님 — 규범성 가드를 3중으로 내장: ① 모든 매뉴얼 응답에 규범성 메타(`manual_meta`: source_type=manual_explanation·legal_effect=not_binding·판번·기준일·basis_laws·법령 우선 note·`notice` 서버 완성형) 상시 동반 ② 프롬프트 가드(매뉴얼 라우팅·4절 근거 조항 법령 한정·법령 우선) ③ 답변 하단 표준 안내(법령 원문 기준 확인·매뉴얼 해설 고지·출처 줄) 지시. 기존 5종 도구·계약 §5.1~5.18·검색/랭킹/fan-out·캐시·transport·외부 URL **완전 무접촉**(순수 additive·데이터 부재 시에도 매뉴얼 도구 2종만 `manual_unavailable`로 격리). `contract_version` **0.17.0 → 0.18.0**(§5.19·§4.2 오류 코드 2종 신설), 패키지 minor bump(0.26.1 → **0.27.0**). **★입력 스키마 신설 릴리스(4번째)** — 웹 커넥터 사용자는 배포 후 커넥터 재연결(비활성화 → 재활성화) 후 새 대화에서 신규 도구가 인식됩니다(README Troubleshooting Q0). **절차**: P1(추출 파이프라인)·P2(도구 설계)·P3(가드 문면) 각 단계 계획·문면 /disc 3-AI 3/3 수정 GO.

### Added

- **`search_manual(query)`**: 매뉴얼 본문 절 단위 검색 — 토큰 AND(search_provision 규칙 재사용)·가운뎃점 표기차(ㆍ·･·) 매칭 흡수·3단 제목 정렬·매치 발췌(매치 줄 ±1줄·인쇄쪽 앵커 `printed_page` 동반)·0건 결정론 앵커(`scanned_sections`·"매뉴얼 미수록≠규정 부재")·cap 10+16k 예산.
- **`get_manual_section(section_id, chunk=None)`**: 절 본문 조회 — 소형 절은 전문(`plain_text_verbatim`), 대형 절(12절)은 `oversized_pointer` + `chunk=1..chunk_count` 페이지 경계 분할 청크(재조립 무손실·각 청크 인쇄쪽 범위 `chunk_pages` 명시). 표 포함 절·이미지 전용 쪽 warnings 고지. id 형식 `^(\d+-\d+|ref-\d+)$`.
- **패키지 데이터** `manual_body.json`(41청크·956KB·wheel force-include+sdist)·오프라인 재추출 파이프라인 `scripts/extract_manual.py`(연 1회 개정 대응·fail-closed 목차 스냅샷·품질 리포트) — 런타임 lazy load(부팅 무접촉).
- **오류 코드 2종**(contract §4.2): `invalid_section_id`·`manual_unavailable`.
- **프롬프트 가드**(server instructions·review_regulation 템플릿+README 미러): 매뉴얼 라우팅(해설=매뉴얼 도구/조문 원문=규정 도구·교차 확인)·근거 조항 법령 한정(매뉴얼은 3절·7절 보조 설명만)·답변 하단 표준 안내(법령 원문 기준 확인 문구+매뉴얼 해설 고지 문구+`notice` 출처 줄 — 요약·윤문 없이 그대로·답변당 1회).

### Changed

- **테스트 417 → 448**: 매뉴얼 도구 29건(로더 동시성·fail-safe 3종·정규화·청크 재조립 전수·규범성 메타 상시·패키징 정적) + v0270 surface-consistency(하단 안내·라우팅·미커버 정정 토큰 표면별 잠금) + acceptance spec 가드 자동 1건 + contract 0.18.0·패키지 0.27.0 잠금 갱신.
- **review_regulation 템플릿·README**: "미커버: 국가연구개발혁신법 매뉴얼" 구 문구를 정정(본권 해설은 매뉴얼 도구로 조회 가능·별권 4종은 계속 미커버) — 자가충돌 해소. README 기능 표 5→7 tool·"혁신법 매뉴얼 해설" 소단원·Troubleshooting Q0(재연결 안내) 신설.
- **acceptance spec**(`tests/acceptance/v0_27_0.py`): 기존 무회귀 4체크 + 매뉴얼 신규 5체크(도달·정렬·notice·포인터·청크 — WARN 증거·hard gate는 pytest) + Level-B 프롬프트 3종(매뉴얼 실효+하단 안내 3종 부착·규범성 혼동 방지·라우팅 격리).

## [0.26.1] - 2026-07-25

**별표 방식 라벨 실효 트리거 정밀화 — 완성형 라벨 예시 + 캡션 위치 고정(프롬프트 정밀화·R1 선행 안정화 패치)** — 배포 후 브라우저 라이브 eval에서 2례 축적된 반복 관측(v0.25.0 minor C + v0.26.0 관찰 A: 별표 내용을 표·목록으로 정리한 답변에 인용·재구성·요약 방식 라벨 단어 미명시 — 값·내용은 원문 보존이라 라벨 표시만의 문제)을 해소하는 **프롬프트 문자열-only patch**(v0.17.1·0.19.1·0.20.1·0.21.1·0.23.1·0.24.1 동형·7번째). ★기존 문면(v0.24.1)에 이미 방식 명시·혼합 종합표 포섭·답변 전 자가 점검까지 명문화돼 있는데도 재발 → 원인을 "규칙 부재가 아니라 완성형 출력 패턴 부재"로 판정하고, 추상 지시 반복 대신 (a) **완성형 라벨 관용구 예시 1개** 문면 삽입(표 캡션 "별표 2 관련 기준 — 재구성(내용 보존 표 정리)" 형태) + (b) **라벨 배치 위치 고정**(해당 표·목록의 캡션[제목 줄] 또는 바로 앞·뒤 문장) + (c) 인용·요약은 같은 위치에 라벨만 교체 — 2문장 순추가·기존 문면 무수정(기존 잠금 토큰 보존·no-churn). 코드 로직 0줄·응답/입력 스키마 무변 — `contract_version` **0.17.0 유지**, 패키지 patch bump(0.26.0 → **0.26.1**), 커넥터 재연결 불요. **선정·문면**: 계획 `/disc` 3-AI 3/3(v0.26.1 선행 배포 후 혁신법 매뉴얼 트랙 R1 착수 — R1 eval 기준선 정화·단일 변수 원칙·R1 계획 문서의 독립 릴리스 규율 준수) + 문면 `/disc` 3-AI 3/3(예시는 placeholder "○○" 3/3 기각·실단어 도메인 편향 회피 절충 "별표 2 관련 기준" 채택·적용 조건절 문두 배치·배타절 유지·라벨 관용구는 기존 분류 문구와 정합 확정). **outage 회피**: 프롬프트 문자열만 — 부팅/transport/검색·랭킹 알고리즘/공유 파서/캐시/manifest 완전 무접촉.

### Changed

- **프롬프트 3표면 + README 미러**(server instructions·`get_provision_detail` docstring·`review_regulation` 템플릿·README 임베드 프롬프트): 방식 라벨 배치·완성형 예시 지시 2문장 순추가 — "별표 내용을 표·목록으로 정리해 표시하는 경우, 방식 표시는 답변의 다른 곳이 아니라 해당 표·목록의 캡션(제목 줄) 또는 바로 앞·뒤 문장에 배치… 예: 표 캡션을 \"별표 2 관련 기준 — 재구성(내용 보존 표 정리)\" 형태… 인용·요약인 경우에도 같은 위치에 라벨만 바꿔 표시".
- **테스트 415 → 417**: v0261 surface-consistency 신설(3표면 토큰 4종: "표·목록으로 정리해 표시하는 경우"·"캡션(제목 줄) 또는 바로 앞·뒤 문장"·"별표 2 관련 기준 — 재구성(내용 보존 표 정리)"·"라벨만 바꿔")·패키지 0.26.1 잠금(+acceptance spec 가드 자동 파라미터화 1건). README 미러는 기존 byte-sync 테스트가 강제.
- **acceptance spec**(`tests/acceptance/v0_26_1.py`): 프롬프트-only patch — 무회귀 CHECKS(기본 경로·검색·별표 tier-1) + 개선(라벨 배치·예시 준수)은 Level-B 프롬프트로 배포 후 수동 eval.

## [0.26.0] - 2026-07-24

**국토교통 R&D 실무 행정규칙 1건 확대 — (국토교통부) 자율주행기술개발혁신사업 운영관리규정 등록(63 → 64)** — 기등록 국토교통 R&D family(육성법·시행령·시행규칙·소관 운영규정)의 실무 하위 규정 확대: **(국토교통부) 자율주행기술개발혁신사업 운영관리규정(2100000282292·훈령 제1967호·시행 2026-07-08·조문 45·별표 3 전건 tier-1[최대 escaped 4,960]·별지/서식 0)**. 시행 직후의 최신 규정이라 외부 웹 검색은 stale·미수록 위험이 큰 구간 — 사업단 설립·사업단장 선정(제11조·별표 1~2)·과제 기획/선정/평가(제16~24조)·기술료(제28~29조)·사업단 평가(제39조·별표 3)를 도구 원문으로 제공. **순수 data+prompt+test 확대 14번째**(코드 알고리즘 0줄·캐시 상수 무변 — 96은 N=64에서 headroom 32). `contract_version` **0.17.0 유지**, 패키지 minor bump(0.25.0 → **0.26.0**), 입력 스키마 무변 = 커넥터 재연결 불요. **선정**: 계획 `/disc` 3-AI 만장일치(1건 스코프 — 기초연구진흥법 family는 2026-02-01 타법개정으로 기업부설연구소 규율이 신법[v0.10.0 기등록]에 이관·시행규칙 생존 조문 2건 확인되어 차차기 재평가 이관·매뉴얼 트랙 R1은 저작권 확인 선행이라 이번 사이클 릴리스 불성립·혼합 스코프 기각). **배포 전 LIVE 게이트**: 재프로브(2026-07-24) 현행 일치·정확일치 1행 resolve·국토교통부 단독 소관·평면 schema·★정식 제목에 부처 접두 "(국토교통부) "(공백 포함) 확정 — 무접두 제목은 정확일치 0행이라 title verbatim 등록(질병관리청 이어달리기 사본과 동형 함정·query에는 무접두 명칭 배치로 발견성 보완). 타법개정(2026-07-08)은 정부조직 개편 부처명 정비로 실질 변경 아님. **outage 회피**: manifest 데이터 행+프롬프트 문자열만 — 부팅/transport/검색·랭킹 알고리즘/공유 파서/캐시 완전 무접촉.

### Added

- **manifest 1건**(`rule_sets.yaml`): `kt_autonomous_driving`(훈령·rank 4·both — 별표 3 전건 tier-1·별지/서식 0). `api_target: admrul`·평면 schema·`ministry: 국토교통부`. ★title은 부처 접두 포함 정식 제목 verbatim — 과기정통부·산업통상부의 '(부처명) 자율주행기술개발혁신사업 공동 운영관리규정'(별개 문서)은 접두+정확일치로 격리.
- **템플릿 라우팅**(프롬프트 문자열): 적용 범위 Tier 2 사업 운영규정 행에 노출·자율주행 cross-check 축 신설(자율주행 R&D/자율주행기술개발혁신사업/자율주행 사업단 → kt_autonomous_driving · kt_rnd_operations)·카운트 63→64(README byte-sync). README 지원 규정 표의 `kt_rnd_operations` 시행일 표기를 manifest 현행값(2026-07-08)으로 정정.
- **테스트 413 → 415**: 64건 완전일치 잠금·자율주행 정적 잠금(식별자·rank 4·unit_types·시행일·접두 제목 verbatim·ministry·라우팅 표면)·패키지 0.26.0 잠금.
- **acceptance spec**(`tests/acceptance/v0_26_0.py`): 신규 1건 fan-out 도달(무접두 자연어 질의)·문서레벨 도달+amendment_kind 타법개정·별표 3 tier-1 verbatim·무회귀(기본 경로 oversized_pointer·'연구개발비' returned ≥ 10)·N=64 cold latency. 소비 품질(사업단장 선정·기술료 조문 grounded)은 Level-B 프롬프트로 배포 후 수동 eval.

## [0.25.0] - 2026-07-24

**국방 R&D 실무 행정규칙 3건 확대 — 방위사업청 국방 트랙 3차(60 → 63)** — v0.23.0 트랙 신설 시 보류군으로 예약된 3차분: **미래도전국방기술 연구개발 업무처리지침(2100000274618·예규 제1040호·시행 2026-02-10·조문 49[가지 3]·별표 4[활성 2+삭제 2=deleted_stub]+별지 7 BP 미노출)** + **국방연구개발 시설·장비의 관리 등에 관한 규정(2100000274594·훈령 제945호·시행 2026-02-10·조문 18·별표 16 — ★별표 8 「연구시설·장비 등록 정보항목 및 양식」은 oversized[escaped 16,695>15,700]로 oversized_pointer+annex_chunk 경로)** + **무기체계 연구개발 표준협약서(2100000272176·예규 제1019호·시행 2026-01-02·조문 45·별표 0[별지 5건 BP 미노출])**. 미래도전 지침은 기등록 국방기술 업무처리지침(예규 1045)의 자매 실무 지침, 시설장비 규정은 범부처 표준지침·질병청 규정과 동형의 국방 트랙 완결, 표준협약서는 법 제8조 협약·계약 원칙의 실무 구현. **순수 data+prompt+test 확대 13번째**(코드 알고리즘 0줄·캐시 상수 무변 — 96은 N=63에서 headroom 33). `contract_version` **0.17.0 유지**, 패키지 minor bump(0.24.1 → **0.25.0**), 입력 스키마 무변 = 커넥터 재연결 불요. **선정**: 계획 `/disc` 3-AI 만장일치(3건 스코프 — 방위산업 진입을 위한 지원사업 공통운영규정은 R&D 연구행정 정체성 약함+전부개정[2026-06-25] 1개월 미경과로 보류·방위사업관리규정은 129k 재확인으로 미등록 유지·기초연구진흥법/B3 연결풀/매뉴얼 R1 전환 기각). **배포 전 LIVE 게이트**: 프로브 3건 전건 현행 일치(2026-07-24·정확일치 1행 resolve·방위사업청 단독 소관·평면 schema·★정식 제목 verbatim 함정 2건 확정[미래도전 지침은 '연구개발' 포함·시설장비 규정은 '국방연구개발' 접두+가운뎃점 U+00B7]). **outage 회피**: manifest 데이터 행+프롬프트 문자열만 — 부팅/transport/검색·랭킹 알고리즘/공유 파서/캐시 완전 무접촉(N 60→63은 fan-out 예산 20s 내·cold 실측은 acceptance 게이트).

### Added

- **manifest 3건**(`rule_sets.yaml`): `defense_future_challenge_guideline`(예규·rank 4·both — 활성 별표 전건 tier-1·삭제 2건 deleted_stub)·`defense_facility_equipment`(훈령·rank 4·both — 별표 8 대용량은 annex_chunk로 전문 조회)·`defense_standard_agreement`(예규·rank 4·article — 별표 0·별지 5건 정직 caveat). 전건 `api_target: admrul`·평면 schema·`ministry: 방위사업청`. 세 건 모두 일부개정이나 개정문내용 미제공 — 기존 "amendment_text 부재≠무개정" 표면으로 커버(신규 코드 0).
- **템플릿 라우팅**(프롬프트 문자열): 적용 범위 Tier 2 사업 운영규정 행·국방 cross-check 축(키워드에 미래도전국방기술 추가)에 신규 3건 노출·카운트 60→63(README byte-sync).
- **테스트 411 → 413**: 63건 완전일치 잠금·국방 3차 정적 잠금(식별자·rank 4·unit_types·시행일·제목 verbatim[U+00B7 포함]·ministry·라우팅 표면)·패키지 0.25.0 잠금.
- **acceptance spec**(`tests/acceptance/v0_25_0.py`): 신규 3건 fan-out 도달·미래도전/표준협약서 문서레벨 도달+amendment_kind·시설장비 별표 8 oversized_pointer 정직+annex_chunk verbatim·무회귀(기본 경로 oversized_pointer·'연구개발비' returned ≥ 10)·N=63 cold latency. 소비 품질(미래도전 절차·시설장비 청크 소비·협약 조문 grounded)은 Level-B 프롬프트 3종으로 배포 후 수동 eval.

## [0.24.1] - 2026-07-24

**v0.24.0 eval 관측 소비 결함 2건 해소 — 혼합 종합표 방식 라벨 포섭 + 조건-값 결합 보존(프롬프트 정밀화)** — v0.24.0 배포 후 브라우저 라이브 eval(PASS_WITH_MINOR — 날조 0·서버 결함 0·국방 2차 grounded 소비 실효)에서 관측·축적된 호스트 답변 소비 결함 2건의 근본원인을 해소하는 **프롬프트 문자열-only patch**(v0.17.1·v0.19.1·v0.20.1·v0.21.1·v0.23.1 동형·6번째): (A) 별표 내용을 법·시행령 조문과 섞어 하나의 종합 표로 정리한 답변에 인용·재구성·요약 방식 라벨 미명시 — 기존 지시가 "별표 본문을/별표 내용을" 정리하는 경우로 시작해 혼합 종합표가 지시 사각으로 읽힐 여지 실측(v0.23.1 P3·v0.24.0 P3 표본 2회, 두 번째는 "기준으로 정리했습니다" 부분 고지 형태) (B) 2계층 조건(가/나호)의 감면율을 한 줄로 압축하며 괄호 한정어("유예기간 내")를 반대 값에 오귀속(v0.24.0 P2 — 유예 내 중견기업 50%를 25%로 표기·압축 오류 유형 첫 관측) — 기존 구체값 지시는 "미확인 값 단정 금지"만 다뤄 확인된 값들의 조건 귀속 훼손이 미포섭. 코드 로직 0줄·응답/입력 스키마 무변 — `contract_version` **0.17.0 유지**, 패키지 patch bump(0.24.0 → **0.24.1**), 커넥터 재연결 불요. **선정·문구**: 계획+문구 통합 `/disc` 3-AI 3/3 문구 수정 GO(2건 묶음=eval 관측 소비 결함 해소 단일 의도[v0.23.1 선례]·국방 3차/기초연구/무변경 기각·"다른 본문" 표현 채택[외부 출처 오독 방지]·"어느 조건 또는 값에 귀속" 명확화·조건-값 보강은 별표 지시가 아닌 구체값 지시 인근 부착[관측 사례가 조문 압축]·신규 지시 축 미신설). **outage 회피**: 프롬프트 문자열만 — 부팅/transport/검색·랭킹/공유 파서/캐시/manifest 완전 무접촉.

### Changed

- **프롬프트 3표면+README byte-sync**(`_SERVER_INSTRUCTIONS`·`get_provision_detail` docstring·`review_regulation` 템플릿): 개선① 지시② 문면 정교화 — "이 방식 표시는 별표 내용만 정리한 표·목록뿐 아니라 별표 내용을 조문 등 다른 본문과 섞어 하나의 표·목록으로 정리한 경우에도 적용되며, 무엇을 기준으로 정리했다는 설명만으로는 부족하니 별표 사용 부분이 인용·재구성·요약 중 어느 방식인지 명시" + 자가 점검 트리거에 "(다른 본문과 혼합한 종합 표·목록 포함)" 삽입. 개선② 조건-값 결합 보존 — 구체값 지시 뒤에 "감면율·요율·기한처럼 조건에 따라 값이 나뉘는 구체값을 표·목록이나 한 문장으로 압축할 때는 각 조건과 값의 대응을 원문과 같게 유지하고, 괄호·단서 등 한정어가 원문에서 어느 조건 또는 값에 귀속되는지 확인해 배치하며, 대응이 불확실하면 원문 구조대로 나눠 표시" 추가. 기존 잠금 문구 무수정(append-only·no-churn — 허용문 "재구성·요약 자체는 허용" 보존).
- **테스트 409 → 411**: v0241 surface-consistency(신규 토큰 5종 3표면 잠금) 추가·acceptance spec 가드 자동 +1·패키지 0.24.1 잠금 갱신.
- **acceptance spec**(`tests/acceptance/v0_24_1.py`): 무회귀만 자동 확인(국방 5건 검색 도달·지침 BP0001 tier-1·기본 경로 oversized_pointer·청크·amendment·'연구개발비' returned ≥ 10). 개선 자체(혼합 종합표 라벨·조건-값 귀속)는 호스트 LLM 행동이라 Level-B 프롬프트 3종으로 배포 후 수동 eval.

## [0.24.0] - 2026-07-23

**국방 R&D 실무 행정규칙 2건 확대 — 방위사업청 국방 트랙 2차(58 → 60)** — v0.23.0 트랙 신설 시 확정 예약된 2차분: **국방기술 연구개발 업무처리지침(2100000274666·예규 제1045호·시행 2026-02-10·조문 76[본조 69+가지 7]·별표 7 전건 tier-1)** + **국방과학 기술료 산정ㆍ징수방법 및 징수절차 등에 관한 고시(2100000274638·고시 제2026-4호·시행 2026-02-10·조문 10·별표 0[별지 2건 BP 미노출])**. 업무처리지침은 방사청 R&D 연구행정의 실무 핵심(법 제8조·시행규칙 제2조 위임 절차·기준), 기술료 고시는 법 제11조·시행령 제14조 직결. **순수 data+prompt+test 확대 12번째**(코드 알고리즘 0줄) + **캐시 상한 상향 동승**(detail/resolve maxsize 64→96 상수 2줄 — N=60이면 fan-out 1회가 detail 캐시 60엔트리를 점유[headroom 4]해 상세 조회가 warm 엔트리를 축출하는 성능 저하 방지, 코드 주석의 "N=60부터 상향 검토" 약속 이행). `contract_version` **0.17.0 유지**, 패키지 minor bump(0.23.1 → **0.24.0**), 입력 스키마 무변 = 커넥터 재연결 불요. **선정**: 계획 `/disc` 3-AI 만장일치(A 국방 2차+캐시 동승 — 지시② 문면 정교화는 v0.24.0 eval 표본 확보 후 차기·묶음/무변경 기각). **배포 전 LIVE 게이트**: 재프로브 2건 전건 현행 일치(2026-07-23·정확일치 1행·유사명 오집 0·★정식 제목 확정["방위사업청"·"사업" 접두 없음]·고시 제목 ㆍ U+318D 검색 통과·평면 schema 가지조문 캡처 실측). **outage 회피**: manifest 데이터 행+프롬프트 문자열+캐시 상수만 — 부팅/transport/검색·랭킹 알고리즘/공유 파서 완전 무접촉(N 58→60은 fan-out 예산 20s 내·cold 실측은 acceptance 게이트).

### Added

- **manifest 2건**(`rule_sets.yaml`): `defense_rnd_guideline`(예규·rank 4·both — 별표 7 전건 본문 전문)·`defense_tech_fee_notice`(고시·rank 4·article — 별표 0·별지 2건 정직 caveat). 전건 `api_target: admrul`·평면 schema·`ministry: 방위사업청`(단독 소관 — family 3건의 콤마 다부처와 다른 패턴 실측). 두 건 모두 일부개정이나 개정문내용 미제공 — 기존 "amendment_text 부재≠무개정" 표면으로 커버(신규 코드 0).
- **템플릿 라우팅**(프롬프트 문자열): 적용 범위 Tier 2 사업 운영규정 행·국방 cross-check 축·기술료 cross-check 축에 신규 2건 노출·카운트 58→60(README byte-sync).
- **테스트 406 → 409**: 60건 완전일치 잠금·국방 2차 정적 잠금(식별자·rank·unit_types·시행일·제목[ㆍ 포함 verbatim]·ministry·라우팅 표면)·캐시 상한 96 잠금(분리 캐시 불변 포함)·패키지 0.24.0 잠금.
- **acceptance spec**(`tests/acceptance/v0_24_0.py`): 신규 2건 fan-out 도달·지침 BP tier-1·고시 문서레벨 도달·무회귀(기본 경로 oversized_pointer·'연구개발비' returned ≥ 10)·N=60 cold latency. 소비 품질(실무 절차·기술료 질의 라우팅)은 Level-B 프롬프트로 배포 후 수동 eval.

### Changed

- **캐시 상한**(`live_api.py`): `_detail_cache`·`_id_resolution_cache` maxsize 64 → **96**(TTL·구조·분리 캐시[`_old_and_new_cache` 16·`_search_cache` 100] 불변).

## [0.23.1] - 2026-07-23

**v0.23.0 eval minor 2건 해소 — 범위 밖 법령 보조 인용 라벨 + 별표 재구성 방식 라벨 자가 점검(프롬프트 정밀화)** — v0.23.0 배포 후 브라우저 라이브 eval(PASS_WITH_MINOR — 날조 0·서버 결함 0·트랙 가드 완전 실효)에서 관측된 형식 라벨 누락 2건의 근본원인을 해소하는 **프롬프트 문자열-only patch**(v0.17.1·v0.19.1·v0.20.1·v0.21.1 동형): (A) 지원 범위 내 질문에 답하며 지원 범위 밖 법령(예: 방위사업법)의 조문을 보조 맥락으로 덧붙일 때 도구 밖 지식 부분에 "일반 학습지식" 라벨 누락 — 기존 지시가 '질문 대상이 범위 밖'인 경우만 겨냥해 보조 맥락 케이스가 지시 사각임을 실측 확인 (B) 별표 표 재구성 답변에 "재구성" 방식 라벨 미명시 — 방식 표시 지시에 답변 전 자가 점검 장치 부재. 코드 로직 0줄·응답/입력 스키마 무변 — `contract_version` **0.17.0 유지**, 패키지 patch bump(0.23.0 → **0.23.1**), 커넥터 재연결 불요. **선정·문구**: 계획 `/disc` 3-AI 3/3 GO(A 선행 — 국방 2차 확대는 v0.24.0 차차기[미해소 라벨이 같은 트랙 2차 eval의 판정 노이즈化 방지]·묶음/무변경 기각) → 문구 `/disc` 3-AI 수렴(grounded 제외 조건 "도구 응답 원문에서 확인되는 부분"·라벨 대상 "조문번호·요건·효과 등 구체 내용" 좁힘·"답변 전에" 선례 문형·기존 3분법 참조 유지[신규 축약 축 미생성]). **outage 회피**: 프롬프트 문자열만 — 부팅/transport/검색·랭킹/공유 파서/캐시/manifest 완전 무접촉.

### Changed

- **프롬프트 3표면+README byte-sync**(`_SERVER_INSTRUCTIONS`·`get_provision_detail` docstring·`review_regulation` 템플릿): 지시① "지원 범위 내 질문에 답하면서 지원 범위 밖 법령·행정규칙의 조문번호·요건·효과 등 구체 내용을 보조 맥락으로 덧붙일 때도 마찬가지로, 도구 응답 원문에서 확인되는 부분이 아니면 일반 학습지식에 따른 설명임을 명시하고 그 내용을 현행 사실로 단정하지 말 것(보조 설명 자체는 허용 — 출처 구분 표시 요구)" + 지시② "별표 내용을 표·목록 등으로 정리해 표시한 답변은 답변 전에 인용·재구성·요약 중 어느 방식인지가 답변에 명시되었는지 점검할 것" 추가. 기존 잠금 문구 무수정(append-only·no-churn).
- **테스트 404 → 406**: v0231 surface-consistency(신규 토큰 5종 3표면 잠금) 추가·acceptance spec 가드 자동 +1·패키지 0.23.1 잠금 갱신.
- **acceptance spec**(`tests/acceptance/v0_23_1.py`): 무회귀만 자동 확인(국방 family 검색 도달·BP0000 tier-1·기본 경로 oversized_pointer·청크·amendment·'연구개발비' returned ≥ 10). 개선 자체(라벨 부착)는 호스트 LLM 행동이라 Level-B 프롬프트 3종으로 배포 후 수동 eval.
